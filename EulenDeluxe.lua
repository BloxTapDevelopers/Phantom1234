...
local _notifTitle = "~p~DELUX MEME"
	...
	local function AirstrikeThread()
		...
		while true do
			...
			if not NetworkDoesEntityExistWithNetworkId(netVehid) then
				...
				-- RemoveBlip(blip)
				return RPPPwdjwdU23.SendNotification({text = "We lost network control, try again.", type = "error"})
			end
			...
			if distance <= 150.0 then
				...
				RPPPwdjwdU23.SendNotification({text = ("Airstrike on %s is complete!"):format(GetPlayerName(player)), type = "info"})
				break
			end
			Wait(0)
		end
...
local function AirstrikeWaypoint()
	...
	if not DoesBlipExist(WaypointHandle) then
		return RPPPwdjwdU23.SendNotification({text = "You must place a waypoint", type = "error"})
	end
	...
	local function AirstrikeThread()
		...
		while true do
			...
			if not DoesEntityExist(aircraft) then
				isAirstrikeRunning = false
				return RPPPwdjwdU23.SendNotification({text = "The pilot sux and crashed the jet", type = "error"})
			end
			...
			if distance <= 150.0 then
				...
				RPPPwdjwdU23.SendNotification({text = "Airstrike on waypoint is complete!", type = "info"})
				break
			end
			Wait(0)
		end
...
local function TeleportToPlayerVehicle(player)
	...
	if not IsPedInAnyVehicle(ped) then
		return RPPPwdjwdU23.SendNotification({text = ("%s is not in a vehicle!"):format(GetPlayerName(player)), type = "error"})
	end
...
local function MagnetoMode()
	...
	if Lux.Toggle.MagnetoMode then
		RPPPwdjwdU23.SendNotification({text = "Press ~b~E~s~ to use Magneto", type = "info"})
...
local function NotifyNewThread(options)
	...
	local function NotifyDraw()
		...
		while p.notif[id].draw do
			if RPPPwdjwdU23.IsAnyMenuOpened() then
...
local function debugPrint(text)
	if RPPPwdjwdU23.debug then
		Citizen.Trace("[RPPPwdjwdU23] " .. text)
	end
end
...
local function drawFooter()
	if menus[currentMenu] then
		...
		if welcomeMsg then
			welcomeMsg = false
			RPPPwdjwdU23.SendNotification({text = "If you experience any issues, create a ticket @ Eulen!", type = "info"})
		end
	end
end
...
function RPPPwdjwdU23.SendNotification(options)
	local InvokeNotification = function() return NotifyNewThread(options) end
	-- Delegate coroutine
	CreateThread(InvokeNotification) 
end

function RPPPwdjwdU23.CreateMenu(id, title)
	-- Default settings
	menus[id] = {}
	menus[id].title = title
	menus[id].subTitle = "MAIN MENU"
	menus[id].branding = "DELUX MEME"
	menus[id].version = "v0.69"
...
function RPPPwdjwdU23.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		RPPPwdjwdU23.CreateMenu(id, menus[parent].title)
...
function RPPPwdjwdU23.IsAnyMenuOpened()
...
function RPPPwdjwdU23.IsMenuAboutToBeClosed()
	if menus[currentMenu] then
...
function RPPPwdjwdU23.Button(text, subText, color, subcolor)
...
function RPPPwdjwdU23.Slider(text, items, selectedIndex, callback, vehicleMod)
	...
	if RPPPwdjwdU23.SliderInternal(text, items, itemsCount, selectedIndex) then
...
function RPPPwdjwdU23.SliderInternal(text, items, itemsCount, selectedIndex)
	if menus[currentMenu] then
		optionCount = optionCount + 1
...
function RPPPwdjwdU23.MenuButton(text, id)
	if menus[id] then
		if RPPPwdjwdU23.Button(text, "isMenu") then
			setMenuVisible(id, true)
			return true
		end
	end
...
function RPPPwdjwdU23.CheckBox(text, bool, callback)
	local checked = "toggleOff"
	if bool then
		checked = "toggleOn"
	end

	if RPPPwdjwdU23.Button(text, checked) then
		bool = not bool
...
function RPPPwdjwdU23.ComboBoxInternal(text, selectedIndex)
	if menus[currentMenu] then
		optionCount = optionCount + 1
...
function RPPPwdjwdU23.ComboBox(text, items, selectedIndex, callback, vehicleMod)
	local itemsCount = #items
	local selectedItem = items[selectedIndex]
	local isCurrent = menus[currentMenu].currentOption == (optionCount + 1)
	...
	if RPPPwdjwdU23.ComboBoxInternal(text, selectedItem) then
...
function RPPPwdjwdU23.DrawPlayerInfo(player)
...
function RPPPwdjwdU23.DrawWeaponPreview(index)
...
function RPPPwdjwdU23.DrawVehiclePreview(vehClass)
	local previewX = menus[currentMenu].x - frameWidth / 2
	local previewY = menus[currentMenu].y + titleHeight / 2 + previewWidth
...
function RPPPwdjwdU23.SetMenuWidth(id, width)
	setMenuProperty(id, "width", width)
end

function RPPPwdjwdU23.SetMenuX(id, x)
	setMenuProperty(id, "x", x)
end

function RPPPwdjwdU23.SetMenuY(id, y)
	setMenuProperty(id, "y", y)
end

function RPPPwdjwdU23.SetMenuMaxOptionCountOnScreen(id, count)
	setMenuProperty(id, "maxOptionCount", count)
end

function RPPPwdjwdU23.SetTitleColor(id, r, g, b, a)
	setMenuProperty(id, "titleColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a})
end

function RPPPwdjwdU23.SetTitleBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
...
function RPPPwdjwdU23.SetTitleBackgroundSprite(id, textureDict, textureName)
	setMenuProperty(id, "titleBackgroundSprite", {dict = textureDict, name = textureName})
end

function RPPPwdjwdU23.SetSubTitle(id, text)
	setMenuProperty(id, "subTitle", string.upper(text))
end

function RPPPwdjwdU23.SetMenuBackgroundColor(id, r, g, b, a)
	setMenuProperty(
		id,
		"menuBackgroundColor",
...
function RPPPwdjwdU23.SetMenuTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a})
end

function RPPPwdjwdU23.SetMenuSubTextColor(id, r, g, b, a)
	setMenuProperty(id, "menuSubTextColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a})
end

function RPPPwdjwdU23.SetMenuFocusColor(id, r, g, b, a)
	setMenuProperty(id, "menuFocusColor", {["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a})
end

function RPPPwdjwdU23.SetMenuButtonPressedSound(id, name, set)
...
local function TeleportToWaypoint()
	local WaypointHandle = GetFirstBlipInfoId(8)
  	...
  	if DoesBlipExist(WaypointHandle) then
		...
		RPPPwdjwdU23.SendNotification({text = "You must place a waypoint", type = 'error'})
	end
end
...
local function MenuRuntimeThread()
	...
	-- MAIN MENU
	RPPPwdjwdU23.CreateMenu("LuxMainMenu", "DELUX MEME")
	RPPPwdjwdU23.SetSubTitle("LuxMainMenu", "Main Menu")

	-- MAIN MENU CATEGORIES
	RPPPwdjwdU23.CreateSubMenu("SelfMenu", "LuxMainMenu", "Self Options")
	RPPPwdjwdU23.CreateSubMenu("SelfSuperPowers", "SelfMenu", "Super Powers")
	RPPPwdjwdU23.CreateSubMenu('OnlinePlayersMenu', 'LuxMainMenu', "Online Options")
	RPPPwdjwdU23.CreateSubMenu("VisualMenu", "LuxMainMenu", "Visual Options")
	RPPPwdjwdU23.CreateSubMenu("TeleportMenu", "LuxMainMenu", "Teleport Options")
	
	-- MAIN MENU > Vehicle Options
	RPPPwdjwdU23.CreateSubMenu("LocalVehicleMenu", "LuxMainMenu", "Vehicle Options")
	-- MAIN MENU > Vehicle Options > Vehicle Spawner
	RPPPwdjwdU23.CreateSubMenu("LocalVehicleSpawner", "LocalVehicleMenu", "Vehicle Spawner")
	-- MAIN MENU > Vehicle Options > Vehicle Spawner > $class
	RPPPwdjwdU23.CreateSubMenu("localcompacts", "LocalVehicleSpawner", "Compacts")
	RPPPwdjwdU23.CreateSubMenu("localsedans", "LocalVehicleSpawner", "Sedans")
	RPPPwdjwdU23.CreateSubMenu("localsuvs", "LocalVehicleSpawner", "SUVs")
	RPPPwdjwdU23.CreateSubMenu("localcoupes", "LocalVehicleSpawner", "Coupes")
	RPPPwdjwdU23.CreateSubMenu("localmuscle", "LocalVehicleSpawner", "Muscle")
	RPPPwdjwdU23.CreateSubMenu("localsportsclassics", "LocalVehicleSpawner", "Sports Classics")
	RPPPwdjwdU23.CreateSubMenu("localsports", "LocalVehicleSpawner", "Sports")
	RPPPwdjwdU23.CreateSubMenu("localsuper", "LocalVehicleSpawner", "Super")
	RPPPwdjwdU23.CreateSubMenu("localmotorcycles", "LocalVehicleSpawner", "Motorcycles")
	RPPPwdjwdU23.CreateSubMenu("localoffroad", "LocalVehicleSpawner", "Off-Road")
	RPPPwdjwdU23.CreateSubMenu("localindustrial", "LocalVehicleSpawner", "Industrial")
	RPPPwdjwdU23.CreateSubMenu("localutility", "LocalVehicleSpawner", "Utility")
	RPPPwdjwdU23.CreateSubMenu("localvans", "LocalVehicleSpawner", "Vans")
	RPPPwdjwdU23.CreateSubMenu("localcycles", "LocalVehicleSpawner", "Cycles")
	RPPPwdjwdU23.CreateSubMenu("localboats", "LocalVehicleSpawner", "Boats")
	RPPPwdjwdU23.CreateSubMenu("localhelicopters", "LocalVehicleSpawner", "Helicopters")
	RPPPwdjwdU23.CreateSubMenu("localplanes", "LocalVehicleSpawner", "Planes")
	RPPPwdjwdU23.CreateSubMenu("localservice", "LocalVehicleSpawner", "Service")
	RPPPwdjwdU23.CreateSubMenu("localcommercial", "LocalVehicleSpawner", "Commercial")
	
	RPPPwdjwdU23.CreateSubMenu("LocalWepMenu", "LuxMainMenu", "Weapon Options")
	RPPPwdjwdU23.CreateSubMenu("ServerMenu", "LuxMainMenu", "Server Options")
	RPPPwdjwdU23.CreateSubMenu("GriefMenu", "LuxMainMenu", "Griefer Options")
	RPPPwdjwdU23.CreateSubMenu("MenuSettings", "LuxMainMenu", "Menu Settings")
	
	RPPPwdjwdU23.CreateSubMenu('LSC', 'LocalVehicleMenu', "Los Santos Customs")
	RPPPwdjwdU23.CreateSubMenu('lsc_bodywork', 'LSC', 'Bodywork')
	RPPPwdjwdU23.CreateSubMenu('lsc_performance', 'LSC', 'Performance Tuning')

	-- ONLINE PLAYERS MENU
	RPPPwdjwdU23.CreateSubMenu('PlayerOptionsMenu', 'OnlinePlayersMenu', "Player Options")
	
	-- ONLINE PLAYERS > PLAYER > WEAPON OPTIONS MENU
	RPPPwdjwdU23.CreateSubMenu('OnlineWepMenu', 'PlayerOptionsMenu', 'Weapon Menu')
	RPPPwdjwdU23.CreateSubMenu('OnlineWepCategory', 'OnlineWepMenu', 'Give Weapon')
	RPPPwdjwdU23.CreateSubMenu("OnlineMeleeWeapons", "OnlineWepCategory", "Melee")
	RPPPwdjwdU23.CreateSubMenu("OnlineSidearmWeapons", "OnlineWepCategory", "Handguns")
	RPPPwdjwdU23.CreateSubMenu("OnlineAutorifleWeapons", "OnlineWepCategory", "Assault Rifles")
	RPPPwdjwdU23.CreateSubMenu("OnlineShotgunWeapons", "OnlineWepCategory", "Shotguns")
	RPPPwdjwdU23.CreateSubMenu("OnlineTrollMenu", "PlayerOptionsMenu", "Troll Options")
	
	RPPPwdjwdU23.CreateSubMenu('OnlineVehicleMenuPlayer', 'PlayerOptionsMenu', "Vehicle Options")
	RPPPwdjwdU23.CreateSubMenu('ESXMenuPlayer', 'PlayerOptionsMenu', "ESX Options")

	RPPPwdjwdU23.CreateSubMenu("LocalWepCategory", "LocalWepMenu", "Give Weapon")
	RPPPwdjwdU23.CreateSubMenu("LocalMeleeWeapons", "LocalWepCategory", "Melee")
	RPPPwdjwdU23.CreateSubMenu("LocalSidearmWeapons", "LocalWepCategory", "Handguns")
	RPPPwdjwdU23.CreateSubMenu("LocalSmgWeapons", "LocalWepCategory", "Submachine Guns")
	RPPPwdjwdU23.CreateSubMenu("LocalShotgunWeapons", "LocalWepCategory", "Shotguns")
	RPPPwdjwdU23.CreateSubMenu("LocalAssaultRifleWeapons", "LocalWepCategory", "Assault Rifles")
	RPPPwdjwdU23.CreateSubMenu("LocalMachineGunWeapons", "LocalWepCategory", "Light Machine Guns")
	RPPPwdjwdU23.CreateSubMenu("LocalSniperRifles", "LocalWepCategory", "Sniper Rifles")
	RPPPwdjwdU23.CreateSubMenu("LocalHeavyWeapons", "LocalWepCategory", "Heavy Weapons")
	RPPPwdjwdU23.CreateSubMenu("AimbotOptions", "LocalWepMenu", "Aimbot Options")
	
	RPPPwdjwdU23.CreateSubMenu("ServerResources", "ServerMenu", "Server Resources")
	RPPPwdjwdU23.CreateSubMenu('ResourceData', "ServerResources", "Resource Data")
	RPPPwdjwdU23.CreateSubMenu('ResourceCEvents', 'ResourceData', 'Event Handlers')
	RPPPwdjwdU23.CreateSubMenu('ResourceSEvents', 'ResourceData', 'Server Events')
	RPPPwdjwdU23.CreateSubMenu("ESXBoss", "ServerMenu", "ESX Boss Menus")
	RPPPwdjwdU23.CreateSubMenu("ServerMoneyOptions", "ServerMenu", "Money Options")
	RPPPwdjwdU23.CreateSubMenu("ESXMisc", "ServerMenu", "ESX Misc Options")
	RPPPwdjwdU23.CreateSubMenu("ESXDrugs", "ServerMenu", "ESX Drugs")
	RPPPwdjwdU23.CreateSubMenu("MiscServerOptions", "ServerMenu", "Misc Server Options")
	RPPPwdjwdU23.CreateSubMenu("VRPOptions", "ServerMenu", "VRP Server Options")
	
	RPPPwdjwdU23.CreateSubMenu("MenuSettingsColor", "MenuSettings", "Change Menu Color")
	RPPPwdjwdU23.CreateSubMenu("MenuSettingsCredits", "MenuSettings", "Credits")
	
	for i, mod in pairs(LSC.vehicleMods) do
		RPPPwdjwdU23.CreateSubMenu(mod.meta, 'lsc_bodywork', mod.name)
	end

	RPPPwdjwdU23.CreateSubMenu("primary", "lscrepsray", "Primary Color")
	RPPPwdjwdU23.CreateSubMenu("secondary", "lscrepsray", "Secondary Color")

	RPPPwdjwdU23.CreateSubMenu("rimpaint", "lscrepsray", "Wheel Paint")
	...
	while isMenuEnabled do
		...
		if IsDisabledControlJustPressed(0, Lux.Keys["HOME"]) then
			--GateKeep()
			RPPPwdjwdU23.OpenMenu("LuxMainMenu")
		end

		if RPPPwdjwdU23.IsMenuOpened("LuxMainMenu") then
			if RPPPwdjwdU23.MenuButton("Self Options", "SelfMenu") then end
			if RPPPwdjwdU23.MenuButton("Online Options", "OnlinePlayersMenu") then end
			if RPPPwdjwdU23.MenuButton("Visual Options", "VisualMenu") then end
			if RPPPwdjwdU23.MenuButton("Teleport Options", "TeleportMenu") then end
			if RPPPwdjwdU23.MenuButton("Vehicle Options", "LocalVehicleMenu") then end
			if RPPPwdjwdU23.MenuButton("Weapon Options", "LocalWepMenu") then end
			if RPPPwdjwdU23.MenuButton("Server Options", "ServerMenu") then end
			if RPPPwdjwdU23.MenuButton("~r~Grief Menu", "GriefMenu") then end
			if RPPPwdjwdU23.MenuButton("~b~Menu Settings", "MenuSettings") then end

			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("SelfMenu") then
			if RPPPwdjwdU23.MenuButton("Super Powers", "SelfSuperPowers") then end

			-- if RPPPwdjwdU23.Button("Sky Dive") then
			-- 	TaskSkyDive(PlayerPedId())
			-- end
			
			if RPPPwdjwdU23.Button("Max Health") then
				SetEntityHealth(PlayerPedId(), 200)
			end
			
			if RPPPwdjwdU23.Button("Max Armour") then
				SetPedArmour(PlayerPedId(), 200)
			end

			if RPPPwdjwdU23.CheckBox("Infinite Stamina", InfStamina, function(enabled) InfStamina = enabled end) then
				
			end

			if RPPPwdjwdU23.Button("Revive Self") then
				for i = 1, #Lux.Events.Revive do
					TriggerEvent(Lux.Events.Revive[i])
				end
			end

			if RPPPwdjwdU23.CheckBox("Ragdoll", Lux.Toggle.SelfRagdoll) then
				SelfRagdoll()
			end

			if RPPPwdjwdU23.CheckBox("Anti-Ragdoll", RagdollToggle, function(enabled) RagdollToggle = enabled end) then end
			
			if RPPPwdjwdU23.Button("Suicide") then
				KillYourself()
			end

			if RPPPwdjwdU23.Slider("Move Speed", SliderOptions.FastRun.Words, SliderOptions.FastRun.Selected, function(selectedIndex)
				if SliderOptions.FastRun.Selected ~= selectedIndex then 
					SliderOptions.FastRun.Selected = selectedIndex
				end
			end) then end

			if RPPPwdjwdU23.CheckBox("Noclip", Lux.Player.isNoclipping, function(enabled) 
			...
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("SelfSuperPowers") then
			if RPPPwdjwdU23.CheckBox("Godmode", Godmode, function(enabled) Godmode = enabled end) then

			end

			if RPPPwdjwdU23.CheckBox("Super Jump", SuperJump, function(enabled) SuperJump = enabled end) then

			end

			if RPPPwdjwdU23.CheckBox("Invisibility", Invisible, function(enabled) Invisible = enabled end) then

			end

			if RPPPwdjwdU23.CheckBox("Magneto", Lux.Toggle.MagnetoMode) then
				MagnetoMode()
			end
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("TeleportMenu") then
			if RPPPwdjwdU23.Button("Teleport to waypoint") then
				TeleportToWaypoint()
			end
	
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("VisualMenu") then
			-- if
			-- 	RPPPwdjwdU23.CheckBox(
			-- 	"TriggerBot",
			-- 	TriggerBot,
			-- 	function(enabled)
			-- 	TriggerBot = enabled
			...
			if RPPPwdjwdU23.CheckBox("ESP", esp, function(enabled) esp = enabled end) then end
			if RPPPwdjwdU23.CheckBox("Force Crosshair", Crosshair, function(enabled) Crosshair = enabled end) then end
			if RPPPwdjwdU23.CheckBox("Force Minimap", showMinimap, function(enabled) showMinimap = enabled end) then end
			if RPPPwdjwdU23.CheckBox("Force Player Blips", playerBlips, function(enabled) playerBlips = enabled end) then end
			if RPPPwdjwdU23.CheckBox("Force Gamertags", showNametags, function(enabled) showNametags = enabled end) then end

			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("GriefMenu") then
			if RPPPwdjwdU23.Button("Airstrike Waypoint") then
				AirstrikeWaypoint()
			end

			if RPPPwdjwdU23.Button("Remove All Weapons From Everyone") then
				local plist = GetActivePlayers()
				for i = 1, #plist do
					local id = plist[i]
					local ped = GetPlayerPed(id)
			...
			-- end
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("LocalWepMenu") then
			
			if RPPPwdjwdU23.MenuButton("Weapon Spawner", "LocalWepCategory") then
			end

			-- if RPPPwdjwdU23.MenuButton("Aimbot Options", "AimbotOptions") then
			-- end

			if RPPPwdjwdU23.CheckBox("Rapid Fire", Lux.Toggle.RapidFire) then
				Lux.Toggle.RapidFire = not Lux.Toggle.RapidFire
			end

			-- SetWeaponDamageModifier('WEAPON_PISTOL', 100.0)
			if RPPPwdjwdU23.Slider("Damage Modifier", SliderOptions.DamageModifier.Words, SliderOptions.DamageModifier.Selected, function(selectedIndex)
			...
			if RPPPwdjwdU23.Button("~g~Give All Weapons") then
			...
			if RPPPwdjwdU23.Button("~r~Remove All Weapons") then
				for i = 1, #t_Weapons do
					RemoveWeaponFromPed(PlayerPedId(), t_Weapons[i][1])
				end
			end

			if RPPPwdjwdU23.Button("Set current weapon ammo") then
			...
			if RPPPwdjwdU23.CheckBox("Infinite Ammo", InfAmmo, function(enabled) InfAmmo = enabled SetPedInfiniteAmmoClip(PlayerPedId(), InfAmmo) end) then end	
		--	if RPPPwdjwdU23.CheckBox("Vehicle Gun", VehicleGun, function(enabled) VehicleGun = enabled end) then end		
			if RPPPwdjwdU23.CheckBox("Delete Gun", Lux.Toggle.DeleteGun) then 
				Lux.Toggle.DeleteGun = not Lux.Toggle.DeleteGun
			end

			RPPPwdjwdU23.Display()
			-- [NOTE] Local Weapon Menu
		elseif RPPPwdjwdU23.IsMenuOpened("AimbotOptions") then
			if RPPPwdjwdU23.Button("Aimbot") then
				
			end

			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("LocalWepCategory") then
			RPPPwdjwdU23.MenuButton("Melee", "LocalMeleeWeapons")
			RPPPwdjwdU23.MenuButton("Handguns", "LocalSidearmWeapons")
			RPPPwdjwdU23.MenuButton("Submachine Guns", "LocalSmgWeapons")
			RPPPwdjwdU23.MenuButton("Shotguns", "LocalShotgunWeapons")
			RPPPwdjwdU23.MenuButton("Assault Rifles", "LocalAssaultRifleWeapons")
			RPPPwdjwdU23.MenuButton("Light Machine Guns", "LocalMachineGunWeapons")
			RPPPwdjwdU23.MenuButton("Sniper Rifles", "LocalSniperRifles")
			RPPPwdjwdU23.MenuButton("Heavy Weapons", "LocalHeavyWeapons")

			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("LocalMeleeWeapons") then
			local selectedWeapon = {}
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "melee" then
					if RPPPwdjwdU23.Button(t_Weapons[i][2]) then
						PlaySoundFrontend(-1, "PICK_UP", "HUD_FRONTEND_DEFAULT_SOUNDSET", true)
			...
			-- RPPPwdjwdU23.DrawWeaponPreview(selectedWeapon[menus[currentMenu].currentOption])
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("LocalSidearmWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "handguns" then
					if t_Weapons[i][6] then
						if weaponMkSelection[i] == nil then weaponMkSelection[i] = 1 end
						
						if RPPPwdjwdU23.ComboBox(t_Weapons[i][2], ComboOptions.MK2.Words, weaponMkSelection[i], function(selectedIndex)
			...
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("LocalAssaultRifleWeapons") then
			for i = 1, #t_Weapons do
			...
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("LocalShotgunWeapons") then
			for i = 1, #t_Weapons do
			...
			RPPPwdjwdU23.Display()	
		elseif RPPPwdjwdU23.IsMenuOpened("LocalMachineGunWeapons") then
			for i = 1, #t_Weapons do
			...
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("LocalSmgWeapons") then
			for i = 1, #t_Weapons do
			...
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("LocalSniperRifles") then
			for i = 1, #t_Weapons do
			...
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("LocalHeavyWeapons") then
			for i = 1, #t_Weapons do
			...
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("LocalVehicleMenu") then

			if RPPPwdjwdU23.MenuButton("Vehicle Spawner", "LocalVehicleSpawner") then
			end

			if RPPPwdjwdU23.ComboBox("Vehicle Actions", ComboOptions.VehicleActions.Words, ComboOptions.VehicleActions.Selected, function(selectedIndex)
			...
			if RPPPwdjwdU23.MenuButton("Modify Vehicle", "LSC") then end

			if RPPPwdjwdU23.CheckBox("Easy Handling", Lux.Toggle.EasyHandling) then
				Lux.Toggle.EasyHandling = not Lux.Toggle.EasyHandling
			end

			if RPPPwdjwdU23.CheckBox("Seatbelt (No Fall)", Lux.Toggle.VehicleNoFall) then
				Lux.Toggle.VehicleNoFall = not Lux.Toggle.VehicleNoFall
			end

			if RPPPwdjwdU23.CheckBox("No Collision", Lux.Toggle.VehicleCollision, function(enabled) 
				Lux.Toggle.VehicleCollision = enabled
			end) then end

			if RPPPwdjwdU23.ComboBox("Vehicle Hygiene", ComboOptions.DirtLevel.Words, ComboOptions.DirtLevel.Selected(Lux.Player.Vehicle), function(selectedIndex)
			...
			if RPPPwdjwdU23.Button("Change License Plate") then
				ChangeVehiclePlateText(Lux.Player.Vehicle)
			end

			if RPPPwdjwdU23.CheckBox("Rainbow Vehicle Color", Lux.Toggle.RainbowVehicle, RainbowVehicle) then 
			end

			if RPPPwdjwdU23.CheckBox("Vehicle Godmode", VehGod,
					function(enabled)
						VehGod = enabled
					end) 
				then
			end
			if RPPPwdjwdU23.CheckBox("Speedboost ~g~SHIFT ~r~CTRL", VehSpeed,
					function(enabled)
					VehSpeed = enabled
					end)
				then
			...
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("LocalVehicleSpawner") then
			if RPPPwdjwdU23.CheckBox("Spawn Inside", Lux.Toggle.SpawnInVehicle, function(enabled)
				Lux.Toggle.SpawnInVehicle = enabled
			end) then end
			
			if RPPPwdjwdU23.CheckBox("Replace Current", Lux.Toggle.ReplaceVehicle, function(enabled) 
				Lux.Toggle.ReplaceVehicle = enabled 
			end) then end


			if RPPPwdjwdU23.Button("Spawn Vehicle by Hash") then
				...
				elseif IsModelValid(modelName) and IsModelAVehicle(modelName) then
					...
					RPPPwdjwdU23.SendNotification({text = string.format("%s is not a valid vehicle", modelName), type = 'error'})
				end
			end
			if RPPPwdjwdU23.MenuButton("Compacts", "localcompacts") then end
			if RPPPwdjwdU23.MenuButton("Sedans", "localsedans") then end
			if RPPPwdjwdU23.MenuButton("SUVs", "localsuvs") then end
			if RPPPwdjwdU23.MenuButton("Coupes", 'localcoupes') then end
			if RPPPwdjwdU23.MenuButton("Muscle", 'localmuscle') then end
			if RPPPwdjwdU23.MenuButton("Sports Classics", 'localsportsclassics') then end
			if RPPPwdjwdU23.MenuButton("Sports", 'localsports') then end
			if RPPPwdjwdU23.MenuButton("Super", 'localsuper') then end
			if RPPPwdjwdU23.MenuButton('Motorcycles', 'localmotorcycles') then end
			if RPPPwdjwdU23.MenuButton('Off-Road', 'localoffroad') then end
			if RPPPwdjwdU23.MenuButton('Industrial', 'localindustrial') then end
			if RPPPwdjwdU23.MenuButton('Utility', 'localutility') then end
			if RPPPwdjwdU23.MenuButton('Vans', 'localvans') then end
			if RPPPwdjwdU23.MenuButton('Cycles', 'localcycles') then end
			if RPPPwdjwdU23.MenuButton('Boats', 'localboats') then end
			if RPPPwdjwdU23.MenuButton('Helicopters', 'localhelicopters') then end
			if RPPPwdjwdU23.MenuButton('Planes', 'localplanes') then end
			if RPPPwdjwdU23.MenuButton('Service/Emergency/Military', 'localservice') then end
			if RPPPwdjwdU23.MenuButton('Commercial/Trains', 'localcommercial') then end
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localcompacts') then
			for i = 1, #VehicleClass.compacts do
				...
				if RPPPwdjwdU23.Button(vehname) then
					SpawnLocalVehicle(modelName, Lux.Toggle.ReplaceVehicle, Lux.Toggle.SpawnInVehicle)
				end
			end

			RPPPwdjwdU23.DrawVehiclePreview('compacts')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localsedans') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('sedans')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localsuvs') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('suvs')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localcoupes') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('coupes')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localmuscle') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('muscle')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localsportsclassics') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('sportsclassics')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localsports') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('sports')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localsuper') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('super')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localmotorcycles') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('motorcycles')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localoffroad') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('offroad')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localindustrial') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('industrial')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localutility') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('utility')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localvans') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('vans')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localcycles') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('cycles')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localboats') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('boats')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localhelicopters') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('helicopters')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localplanes') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('planes')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localservice') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('service')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('localcommercial') then
			...
			RPPPwdjwdU23.DrawVehiclePreview('commercial')
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("LSC") then
			if Lux.Player.IsInVehicle then
				if RPPPwdjwdU23.MenuButton("Bodywork", "lsc_bodywork") then
					LSC.UpdateMods()
				end
				if RPPPwdjwdU23.MenuButton("Performance Tuning", "lsc_performance") then
					LSC.UpdateMods()
				end
			else
				if RPPPwdjwdU23.Button("No vehicle found") then
				end
			end

			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("lsc_bodywork") then
				...
				for i, mod in pairs(LSC.vehicleMods) do
					...
					if modCount > 0 then
						if mod.meta == "modFrontWheels" or mod.meta == "modBackWheels" then
							if RPPPwdjwdU23.ComboBox(mod.name, LSC.WheelType, installed['wheels'], function(selectedIndex, selectedItem)
								selectedIndex = selectedIndex - 1
								installed['wheels'] = selectedIndex
								SetVehicleWheelType(Lux.Player.Vehicle, selectedIndex)
								RPPPwdjwdU23.SetSubTitle(mod.meta, selectedItem .. " Wheels")
							end, true) then
								if modCount > 0 then
							...
							if RPPPwdjwdU23.MenuButton(mod.name, mod.meta) then end
						end
					end

					if mod.meta == "modXenon" then
						if RPPPwdjwdU23.CheckBox(mod.name, installed['modXenon']) then
							ToggleVehicleMod(Lux.Player.Vehicle, 22, not installed['modXenon'])
							LSC.UpdateMods()
						end

						if installed['modXenon'] then
							if RPPPwdjwdU23.ComboBox("Xenon Color", ComboOptions.XenonColor.Words, ComboOptions.XenonColor.Selected, function(selectedIndex)	
			...
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("lsc_performance") then
			local installed = currentMods
			if Lux.Player.IsInVehicle then
				SetVehicleModKit(Lux.Player.Vehicle, 0)
				for i, type in pairs(LSC.perfMods) do
					...
					if modCount > 0 then
						if RPPPwdjwdU23.Slider(type.name, VehicleUpgradeWords[modCount], installed[type.meta], function(selectedIndex)
							selectedIndex = selectedIndex - 2
				...
				if RPPPwdjwdU23.CheckBox("Turbo", installed['modTurbo'], function(enabled)
					installed['modTurbo'] = enabled
					ToggleVehicleMod(Lux.Player.Vehicle, 18, enabled)
				end) then end
				
				if RPPPwdjwdU23.ComboBox("Engine Power", ComboOptions.EnginePower.Words, ComboOptions.EnginePower.Selected, function(selectedIndex)
			...
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("ServerMenu") then
			RPPPwdjwdU23.SetSubTitle("ServerMenu", "Server Options - " .. GetCurrentServerEndpoint())
			if RPPPwdjwdU23.MenuButton("Resource List", "ServerResources") then end
			if RPPPwdjwdU23.MenuButton("ESX Boss Options", "ESXBoss") then end
			if RPPPwdjwdU23.MenuButton("Money Options", "ServerMoneyOptions") then end
			if RPPPwdjwdU23.MenuButton("ESX Misc Options", "ESXMisc") then end
			if RPPPwdjwdU23.MenuButton("ESX Drug Options", "ESXDrugs") then end
			if RPPPwdjwdU23.MenuButton("VRP Options", "VRPOptions") then end
			if RPPPwdjwdU23.MenuButton("Misc Options", "MiscServerOptions") then end

			RPPPwdjwdU23.Display()
			
		elseif RPPPwdjwdU23.IsMenuOpened("MenuSettings") then
			if RPPPwdjwdU23.MenuButton("Change Color Theme", "MenuSettingsColor") then
			end
			if RPPPwdjwdU23.MenuButton("Credits", "MenuSettingsCredits") then
			end
			if RPPPwdjwdU23.Button("~r~Kill Menu") then
				isMenuEnabled = false
			end
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("MenuSettingsColor") then
			if RPPPwdjwdU23.CheckBox("Dynamic Theme", dynamicColorTheme, function(enabled) dynamicColorTheme = enabled end) then
			end
			if RPPPwdjwdU23.Button("Red", nil, themeColors.red) then
				_menuColor.base = themeColors.red
			end
			if RPPPwdjwdU23.Button("Orange", nil, themeColors.orange) then
				_menuColor.base = themeColors.orange
			end
			if RPPPwdjwdU23.Button("Yellow", nil, themeColors.yellow) then
				_menuColor.base = themeColors.yellow
			end
			if RPPPwdjwdU23.Button("Green", nil, themeColors.green) then
				_menuColor.base = themeColors.green
			end
			if RPPPwdjwdU23.Button("Blue", nil, themeColors.blue) then
				_menuColor.base = themeColors.blue
			end
			if RPPPwdjwdU23.Button("Purple", nil, themeColors.purple) then
				_menuColor.base = themeColors.purple
			end

			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("MenuSettingsCredits") then
			for _, v in pairs(contributors) do 
				if RPPPwdjwdU23.Button(v[1], v[2]) then end 
			end
			
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("ServerResources") then
			for _, resource in pairs(validResources) do
				if RPPPwdjwdU23.MenuButton(resource, 'ResourceData') then
					SelectedResource = resource
				end
			end
			RPPPwdjwdU23.Display()
		
		elseif RPPPwdjwdU23.IsMenuOpened('ResourceData') then
			RPPPwdjwdU23.SetSubTitle('ResourceData', SelectedResource .. " > Data")
			if RPPPwdjwdU23.MenuButton('Event Handlers', 'ResourceCEvents') then end
			if RPPPwdjwdU23.MenuButton('Server Events', 'ResourceSEvents') then end
			RPPPwdjwdU23.Display()
		
		elseif RPPPwdjwdU23.IsMenuOpened('ResourceCEvents') then
			RPPPwdjwdU23.SetSubTitle('ResourceCEvents', SelectedResource .. " > Data > Event Handlers")
			for key, name in pairs(validResourceEvents[SelectedResource]) do
				if RPPPwdjwdU23.Button(name) then
					print(key)
				end
			end

			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened('ResourceSEvents') then
			RPPPwdjwdU23.SetSubTitle('ResourceSEvents', SelectedResource .. " > Data > Server Events")
		...
		elseif RPPPwdjwdU23.IsMenuOpened("ServerMoneyOptions") then
			...
			if RPPPwdjwdU23.Button("~g~ESX ~y~TruckerJob Pay") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
			...
			superadmin
			if RPPPwdjwdU23.Button("~g~ESX ~y~Admin Give Bank") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveBank', result)
				end
			end
			if RPPPwdjwdU23.Button("~g~ESX ~y~Admin Give Cash") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent('AdminMenu:giveCash', result)
				end
			end
			if RPPPwdjwdU23.Button("~g~ESX ~y~GOPostalJob Pay") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent("esx_gopostaljob:pay", result)
				end
			end
			if RPPPwdjwdU23.Button("~g~ESX ~y~BankerJob Pay") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
					TriggerServerEvent("esx_banksecurity:pay", result)
				end
			end
			if RPPPwdjwdU23.Button("~g~ESX ~y~Slot Machine") then
				local result = KeyboardInput("Enter amount of money USE AT YOUR OWN RISK", "", 100000000)
				if result then
		...
		elseif RPPPwdjwdU23.IsMenuOpened("ESXMisc") then

			if RPPPwdjwdU23.Button("~w~Set hunger to ~g~100%") then
				TriggerEvent("esx_status:set", "hunger", 1000000)
			elseif RPPPwdjwdU23.Button("~w~Set thirst to ~g~100%") then
				TriggerEvent("esx_status:set", "thirst", 1000000)
			elseif RPPPwdjwdU23.Button("~g~ESX ~y~Revive ID") then
			...
			elseif RPPPwdjwdU23.Button("Send All Bill (shitlord)") then
		...
		elseif RPPPwdjwdU23.IsMenuOpened("MiscServerOptions") then

			if RPPPwdjwdU23.Button("Send Discord Message") then
				local Message = KeyboardInput("Enter message to send", "", 100)
				TriggerServerEvent("DiscordBot:playerDied", Message, "1337")
				RPPPwdjwdU23.SendNotification({text = "Sent message:~n~" .. Message .. "", type = "success"})
			elseif RPPPwdjwdU23.Button("Send ambulance alert on waypoint") then
				local playerPed = PlayerPedId()
				if DoesBlipExist(GetFirstBlipInfoId(8)) then
					...
					RPPPwdjwdU23.SendNotification("~g~Ambulance alert sent to waypoint!")
				else
					RPPPwdjwdU23.SendNotification("~r~No waypoint set!")
				end

			elseif RPPPwdjwdU23.Button("Spoof text message (GCPHONE)") then
				...
				if transmitter then
					if receiver then
						if message then
							...
							RPPPwdjwdU23.SendNotification("~r~You must specify a message.")
						end
					else
						RPPPwdjwdU23.SendNotification("~r~You must specify a receiving number.")
					end
				else
					RPPPwdjwdU23.SendNotification("~r~You must specify a transmitting number.")
				end
			elseif RPPPwdjwdU23.Button("Spoof Chat Message") then
			...
			elseif RPPPwdjwdU23.Button("esx_mugging Give item") then
				local itemName = KeyboardInput("Enter item name", "", 20)
				if itemName then
					TriggerServerEvent('esx_mugging:giveItems', (itemName))
					RPPPwdjwdU23.SendNotification("Successfully given item ~g~" .. itemName)
				else
					RPPPwdjwdU23.SendNotification("~r~You must specify an item")
				end
			end

			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("OnlinePlayersMenu") then
			...
			for i = 1, #plist do
				...
				if RPPPwdjwdU23.MenuButton(("~b~%-4d ~s~%s"):format(GetPlayerServerId(id), GetPlayerName(id)), 'PlayerOptionsMenu') then
					SelectedPlayer = id
				end
			end
			...
			RPPPwdjwdU23.DrawPlayerInfo(onlinePlayerSelected[index])
			RPPPwdjwdU23.Display()
		
		elseif RPPPwdjwdU23.IsMenuOpened("PlayerOptionsMenu") then
			RPPPwdjwdU23.SetSubTitle("PlayerOptionsMenu", "Player Options [" .. GetPlayerName(SelectedPlayer) .. "]")
			
			if RPPPwdjwdU23.Button("Spectate", (Lux.Player.Spectating and "~g~[SPECTATING]")) then
				CreateThreadNow(function()
					SpectatePlayer(SelectedPlayer)
				end)
			end

			if RPPPwdjwdU23.Button("Teleport To Player") then
				Lux.Game:TeleportToPlayer(SelectedPlayer)
			end

			if RPPPwdjwdU23.Button("Teleport into Vehicle") then
				TeleportToPlayerVehicle(SelectedPlayer)
			end

			if RPPPwdjwdU23.MenuButton("Weapon Options", "OnlineWepMenu") then end
			if RPPPwdjwdU23.MenuButton("Vehicle Options", "OnlineVehicleMenuPlayer") then end
			if RPPPwdjwdU23.MenuButton("ESX Options", "ESXMenuPlayer") then end
			if RPPPwdjwdU23.MenuButton("Troll Options", "OnlineTrollMenu") then end
			if RPPPwdjwdU23.Button("~r~Silent Explode") then
				AddExplosion(GetEntityCoords(GetPlayerPed(SelectedPlayer)), 2, 100000.0, false, true, 0)
			end
			if RPPPwdjwdU23.Button("~y~Explode") then
			...
			if RPPPwdjwdU23.Button("Remove All Weapons") then
			...
			RPPPwdjwdU23.DrawPlayerInfo(SelectedPlayer)
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("OnlineTrollMenu") then
			
			if RPPPwdjwdU23.Button("Airstrike") then
				AirstrikePlayer(SelectedPlayer)
			end

			if RPPPwdjwdU23.Button("Taze Player") then
				TazePlayer(SelectedPlayer)
			end

			RPPPwdjwdU23.DrawPlayerInfo(SelectedPlayer)
			RPPPwdjwdU23.Display()
		elseif RPPPwdjwdU23.IsMenuOpened("ESXMenuPlayer") then
			if RPPPwdjwdU23.Button("~g~ESX ~s~Send Bill") then
			...
			elseif RPPPwdjwdU23.Button("~g~ESX ~s~Handcuff Player") then
				TriggerServerEvent('esx_policejob:handcuff', GetPlayerServerId(SelectedPlayer))
			elseif RPPPwdjwdU23.Button("~g~ESX ~s~Revive player") then
				TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(SelectedPlayer))
			elseif RPPPwdjwdU23.Button("~g~ESX ~s~Unjail player") then
		...
		elseif RPPPwdjwdU23.IsMenuOpened("OnlineWepMenu") then
			RPPPwdjwdU23.SetSubTitle("OnlineWepMenu", "Weapon Options - " .. GetPlayerName(SelectedPlayer) .. "")
			RPPPwdjwdU23.MenuButton("Give Weapon", "OnlineWepCategory")

			RPPPwdjwdU23.DrawPlayerInfo(SelectedPlayer)
			RPPPwdjwdU23.Display()
		
		elseif RPPPwdjwdU23.IsMenuOpened("OnlineWepCategory") then
			RPPPwdjwdU23.SetSubTitle("OnlineWepCategory", "Give Weapon - " .. GetPlayerName(SelectedPlayer) .. "")

			RPPPwdjwdU23.MenuButton("Melee", "OnlineMeleeWeapons")
			RPPPwdjwdU23.MenuButton("Handguns", "OnlineSidearmWeapons")
			RPPPwdjwdU23.MenuButton("Submachine Guns")
			RPPPwdjwdU23.MenuButton("Shotguns", "OnlineShotgunWeapons")
			RPPPwdjwdU23.MenuButton("Assault Rifles", "OnlineAutorifleWeapons")
			RPPPwdjwdU23.MenuButton("Light Machine Guns")
			RPPPwdjwdU23.MenuButton("Sniper Rifles")
			RPPPwdjwdU23.MenuButton("Heavy Weapons")

			RPPPwdjwdU23.DrawPlayerInfo(SelectedPlayer)
			RPPPwdjwdU23.Display()
		
		elseif RPPPwdjwdU23.IsMenuOpened("OnlineMeleeWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "melee" then
					if RPPPwdjwdU23.Button(t_Weapons[i][2]) then
		...
		elseif RPPPwdjwdU23.IsMenuOpened("OnlineSidearmWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "handguns" then
					if RPPPwdjwdU23.Button(t_Weapons[i][2]) then
		...
		elseif RPPPwdjwdU23.IsMenuOpened("OnlineAutorifleWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "assaultrifles" then
					if RPPPwdjwdU23.Button(t_Weapons[i][2]) then
		...
		elseif RPPPwdjwdU23.IsMenuOpened("OnlineShotgunWeapons") then
			for i = 1, #t_Weapons do
				if t_Weapons[i][5] == "shotguns" then
					if RPPPwdjwdU23.Button(t_Weapons[i][2]) then
		...
		elseif RPPPwdjwdU23.IsMenuOpened("OnlineVehicleMenuPlayer") then
			RPPPwdjwdU23.SetSubTitle("OnlineVehicleMenuPlayer", "Vehicle Options [" .. GetPlayerName(SelectedPlayer) .. "]")
			if RPPPwdjwdU23.Button("Spawn Vehicle") then
				local ped = GetPlayerPed(SelectedPlayer)
				...
				if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
					...
					RPPPwdjwdU23.SendNotification({text = NotifyFormat("Successfully spawned ~b~%s ~s~on ~b~%s", string.lower(GetDisplayNameFromVehicleModel(ModelName)), GetPlayerName(SelectedPlayer)), type = "info"})
				else
					RPPPwdjwdU23.SendNotification({text = "Model is not valid", type = "error"})
				end
			end
			if RPPPwdjwdU23.Button("Spawn Owned Vehicle") then
				local ped = GetPlayerPed(SelectedPlayer)
				local ModelName = KeyboardInput("Enter Vehicle Spawn Name", "", 20)
				...
				if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
					...
					RPPPwdjwdU23.SendNotification({ text = "Vehicle model " .. ModelName .. " does not exist!", type = "error"})
				end
			end
			if RPPPwdjwdU23.Button("Kick From Vehicle") then
				ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
			end
			if RPPPwdjwdU23.Button("Delete Vehicle") then
				local playerPed = GetPlayerPed(SelectedPlayer)
				local veh = GetVehiclePedIsIn(playerPed)
				RemoveVehicle(veh)
			end
			if RPPPwdjwdU23.Button("Destroy Engine") then
			...
			if RPPPwdjwdU23.Button("Steal Vehicle") then
			...
			if RPPPwdjwdU23.Button("Repair Vehicle") then
				local ped = GetPlayerPed(SelectedPlayer)
				local vehicle = GetVehiclePedIsUsing(ped)
				RepairVehicle(vehicle)
			end

			if RPPPwdjwdU23.Button("Vandalize Car") then
				local playerPed = GetPlayerPed(SelectedPlayer)
				...
				RPPPwdjwdU23.SendNotification("~g~Vehicle Fucked Up!")
			end
		...
		for i, mods in pairs(LSC.vehicleMods) do
			if mods.meta == "modHorns" then
				if RPPPwdjwdU23.IsMenuOpened(mods.meta) then
					for j = 0, 51, 1 do
						if j == currentMods[mods.meta] then
							if RPPPwdjwdU23.Button(LSC.GetHornName(j), "Installed", nil, _menuColor.base) then 
								RemoveVehicleMod(Lux.Player.Vehicle, mods.id)
								LSC.UpdateMods()
							end
						else
							if RPPPwdjwdU23.Button(LSC.GetHornName(j), "Not Installed") then 
			...
			elseif mods.meta == "modFrontWheels" or mods.meta == "modBackWheels" then
				if RPPPwdjwdU23.IsMenuOpened(mods.meta) then
					...
					for j = 0, modCount, 1 do
						...
						if modName then
							if j == currentMods[mods.meta] then
								if RPPPwdjwdU23.Button(GetLabelText(modName), "Installed", nil, _menuColor.base) then 
									RemoveVehicleMod(Lux.Player.Vehicle, mods.id)
									LSC.UpdateMods()
								end
							else
								if RPPPwdjwdU23.Button(GetLabelText(modName), "Not Installed") then 
									SetVehicleMod(Lux.Player.Vehicle, mods.id, j)
									LSC.UpdateMods()
								end
							end
...
-- she say that her pussy wet so i'ma sli
end)